...
public class AutorouteEngine {
		...
		if (this.maintain_database) {
			if (p_net_no != this.net_no) {
				if (this.complete_expansion_rooms != null) {
					// invalidate the net dependent complete free space
					// expansion rooms.
	...
	public AutorouteResult autoroute_connection(Set<Item> p_start_set, Set<Item> p_dest_set, AutorouteControl p_ctrl,
		...
		if (maze_search_algo != null) {
			try {
				...
				System.out
						.println("AutorouteEngine.autoroute_connection: Exception in maze_search_algo.find_connection");
			}
		}
	...
	public void draw(java.awt.Graphics p_graphics, boardgraphics.GraphicsContext p_graphics_context,
		...
		// this.drill_page_array.draw(p_graphics, p_graphics_context,
		// p_intensity);
	}
	...
	public Collection<CompleteFreeSpaceExpansionRoom> complete_expansion_room(IncompleteFreeSpaceExpansionRoom p_room) {
		...
		try {
			...
			Collection<ExpansionDoor> room_doors = p_room.get_doors();// 这里获取door，是获取的什么door？获取的door为初始化this时在init_connection时加入的；
			for (ExpansionDoor curr_door : room_doors) {
				...
				if (other_room instanceof CompleteFreeSpaceExpansionRoom && curr_door.dimension == 2) {
					from_door_shape = curr_door.get_shape();// 当获取了房间之后，这里回去形状，一般是intOctagon；
			...
			while (it.hasNext()) {
				...
				if (is_first_completed_room) {
					...
					// the shape of the first completed room may have changed
					// and may
					// intersect now with the other shapes. Therefore the
					// completed shapes
	...
	public void complete_neigbour_rooms(CompleteExpansionRoom p_room) {
		...
		while (it.hasNext()) {
			...
			// cast to ExpansionRoom becaus ExpansionDoor.other_room works
			// differently with
			// parameter type CompleteExpansionRoom.
			ExpansionRoom neighbour_room = curr_door.other_room((ExpansionRoom) p_room);// 如果输入的room为first
																						// room，那么就返回second room；反之亦然；
			if (neighbour_room != null) {
				if (neighbour_room instanceof IncompleteFreeSpaceExpansionRoom) {// 是CompleteFreeSpaceExpansionRoom
