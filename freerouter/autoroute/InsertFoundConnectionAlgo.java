...
public class InsertFoundConnectionAlgo {
	...
	public static InsertFoundConnectionAlgo get_instance(LocateFoundConnectionAlgo p_connection, RoutingBoard p_board,
		...
		if (p_connection.target_item instanceof PolylineTrace) {
			...
			p_board.connect_to_trace(new_instance.first_corner, to_trace,
					p_ctrl.trace_half_width[p_connection.start_layer], p_ctrl.trace_clearance_class_no);
		}
		if (p_connection.start_item instanceof PolylineTrace) {
			PolylineTrace to_trace = (PolylineTrace) p_connection.start_item;
			p_board.connect_to_trace(new_instance.last_corner, to_trace,
					p_ctrl.trace_half_width[p_connection.target_layer], p_ctrl.trace_clearance_class_no);
		}
	...
	private boolean insert_trace(LocateFoundConnectionAlgoAnyAngle.ResultItem p_trace) {
		...
		// switch off correcting connection to pin because it may get wrong in
		...
		// Look for pins att the start and the end of p_trace in case that
		// neckdown is necessecary.
		...
		for (int i = 1; i < p_trace.corners.length; ++i) {
			...
			Point ok_point = board.insert_forced_trace_polyline(insert_polyline, ctrl.trace_half_width[p_trace.layer],
					p_trace.layer, net_no_arr, ctrl.trace_clearance_class_no, ctrl.max_shove_trace_recursion_depth,
			...
			if (ok_point == insert_polyline.last_corner() || neckdown_inserted) {
				...
				// if ok_point == insert_polyline.first_corner() the spring over
				// may have failed.
				// Spring over may correct the situation because an insertion,
				// which is ok with clearance compensation
				// may cause violations without clearance compensation.
				// In this case repeating the insertion with more distant
				// corners may allow the spring_over to correct the situation.
				if (from_corner_no > 0) {
					// p_trace.corners[i] may be inside the offset for the
					// substitute trace around
					// a spring_over obstacle (if clearance compensation is
					// off).
	...
	boolean insert_neckdown(Point p_from_corner, Point p_to_corner, int p_layer, board.Pin p_start_pin,
			board.Pin p_end_pin) {
		if (p_start_pin != null) {
	...
	private Point try_neck_down(Point p_from_corner, Point p_to_corner, int p_layer, board.Pin p_pin,
		...
		double curr_clearance = this.board.rules.clearance_matrix.value(ctrl.trace_clearance_class_no,
				p_pin.clearance_class_no(), p_layer);
		...
		if (ok_length <= TOLERANCE) {
			...
			// add a corner in case neck_down_end_point is not exactly on the
			// line from p_from_corner to p_to_corner
			...
			IntPoint add_corner = LocateFoundConnectionAlgo.calculate_additional_corner(float_from_corner,
					float_neck_down_end_point, horizontal_first, board.rules.get_trace_angle_restriction()).round();
			Point curr_ok_point = board.insert_forced_trace_segment(p_from_corner, add_corner,
			...
			curr_ok_point = board.insert_forced_trace_segment(add_corner, neck_down_end_point,
					ctrl.trace_half_width[p_layer], p_layer, net_no_arr, ctrl.trace_clearance_class_no,
					ctrl.max_shove_trace_recursion_depth, ctrl.max_shove_via_recursion_depth,
					ctrl.max_spring_over_recursion_depth, Integer.MAX_VALUE, ctrl.pull_tight_accuracy, true, null);
			...
			add_corner = LocateFoundConnectionAlgo.calculate_additional_corner(float_neck_down_end_point,
					float_to_corner, !horizontal_first, board.rules.get_trace_angle_restriction()).round();
		...
		Point ok_point = board.insert_forced_trace_segment(neck_down_end_point, p_to_corner, neck_down_halfwidth,
				p_layer, net_no_arr, ctrl.trace_clearance_class_no, ctrl.max_shove_trace_recursion_depth,
				ctrl.max_shove_via_recursion_depth, ctrl.max_spring_over_recursion_depth, Integer.MAX_VALUE,
				ctrl.pull_tight_accuracy, true, null);
		return ok_point;
	}
