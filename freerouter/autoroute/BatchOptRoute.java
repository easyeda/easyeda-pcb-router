...
public class BatchOptRoute {
	...
	public void optimize_board() {
		if (routing_board.get_test_level() != TestLevel.RELEASE_VERSION) {
			System.out.println("Before optimize: Via count: " + routing_board.get_vias().size() + ", trace length: "
		...
		while (route_improved) {
			++curr_pass_no;
			boolean with_prefered_directions = (curr_pass_no % 2 != 0); // to
																		// create
																		// more
																		// variations
			route_improved = opt_route_pass(curr_pass_no, with_prefered_directions);
			if (curr_pass_no == AutorouteSettings.maxRouteRetry) {
				break;
			}
		}
	}
	...
	private boolean opt_route_pass(int p_pass_no, boolean p_with_prefered_directions) {
		...
		if (this.use_increased_ripup_costs && !route_improved) {
			...
			route_improved = true; // to keep the optimizer going with lower
									// ripup costs
		}
		return route_improved;
	}
	...
	private boolean opt_route_item(Item p_item, int p_pass_no, boolean p_with_prefered_directions) {
		...
		if (p_item instanceof Trace) {
			// add also the fork items, especially because not all fork items
			// may be
		...
		boolean route_improved = !this.thread.is_stop_requested() && (incomplete_count_after < incomplete_count_before
				...
				&& (via_count_after < via_count_before || via_count_after == via_count_before
		...
		if (route_improved) {
			if (incomplete_count_after < incomplete_count_before
				...
				// Catch unexpected increase of cumulative trace length
				// somewhere for examole by removing acid trapsw.
				this.min_cumulative_trace_length_before = Math.min(this.min_cumulative_trace_length_before,
						trace_length_after);
			}
			...
			if (curr_item instanceof Trace) {
				...
				if (fixed_state == FixedState.UNFIXED || fixed_state == FixedState.SHOVE_FIXED) {
					double weighted_trace_length = curr_trace.get_length()
							* (curr_trace.get_half_width() + p_board.clearance_value(curr_trace.clearance_class_no(),
									default_clearance_class, curr_trace.get_layer()));
	...
	private boolean use_increased_ripup_costs; // in the first passes the ripup
												// costs are icreased for better
												// performance.
	private double min_cumulative_trace_length_before = 0;
	private static int MAX_AUTOROUTE_PASSES = AutorouteSettings.maxRouteRetry;
			...
			for (;;) {
				...
				if (curr_item instanceof Via) {
					...
					if (!curr_via.is_user_fixed()) {
						...
						if (curr_via_center.x > min_item_coor.x || curr_via_center.x == min_item_coor.x
								&& (curr_via_center.y > min_item_coor.y || curr_via_center.y == min_item_coor.y
										&& curr_via_min_layer > min_item_layer)) {
							if (curr_via_center.x < curr_min_coor.x || curr_via_center.x == curr_min_coor.x
									&& (curr_via_center.y < curr_min_coor.y || curr_via_center.y == curr_min_coor.y
					...
					if (!curr_trace.is_shove_fixed()) {
						...
						if (compare_corner.x > min_item_coor.x
							...
							if (compare_corner.x < curr_min_coor.x || compare_corner.x == curr_min_coor.x
									&& (compare_corner.y < curr_min_coor.y || compare_corner.y == curr_min_coor.y
