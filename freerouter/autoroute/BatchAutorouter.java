...
public class BatchAutorouter {
	 ...
	 * the optimize via batch pass. Returns the number of oasses to complete the
	 * board or p_max_pass_count + 1, if the board is not completed.
	 */
	public static int autoroute_passes_for_optimizing_item(InteractiveActionThread p_thread, int p_max_pass_count,
			int p_ripup_costs, boolean p_with_prefered_directions) {
		BatchAutorouter router_instance = new BatchAutorouter(p_thread, true, p_with_prefered_directions,
				p_ripup_costs);
		...
		while (still_unrouted_items && !router_instance.is_interrupted && curr_pass_no <= p_max_pass_count) {
			...
			if (still_unrouted_items && !router_instance.is_interrupted) {
				...
				if (p_thread.hdlg.reduceOptTimes() == 0) {
	...
	public boolean autoroute_passes() {
		...
		while (still_unrouted_items && !this.is_interrupted) {
			if (thread.is_stop_requested()) {
				...
				int curr_pass_no = hdlg.settings.autoroute_settings.get_pass_no();
		...
		if (!(this.remove_unconnected_vias || still_unrouted_items || this.is_interrupted)) {
			// clean up the route if the board is completed and if fanout is
			// used.
	...
	private boolean autoroute_pass(int p_pass_no, boolean p_with_screen_message) {
		try {
			...
			Set<Item> handeled_items = new TreeSet<Item>();// 获取有网络的元器件
			...
			Collection<Item> userFixedItemList = new java.util.LinkedList<Item>();
			for (Item item : autoroute_item_list) {
				if (item.get_fixed_state().equals(FixedState.USER_FIXED)) {
					userFixedItemList.add(item);
				}
			}
			autoroute_item_list.removeAll(userFixedItemList);
	...
	private boolean autoroute_item(Item p_item, int p_route_net_no, SortedSet<Item> p_ripped_item_list,
		...
		try {
			...
			AutorouteControl autoroute_control = new AutorouteControl(this.routing_board, p_route_net_no, hdlg.settings,
					curr_via_costs, this.trace_cost_arr);
