...
public class SortedRoomNeighbours {
	...
	public static CompleteExpansionRoom calculate(ExpansionRoom p_room, AutorouteEngine p_autoroute_engine) {
		...
		SortedRoomNeighbours room_neighbours = calculate_neighbours(p_room, net_no,
				p_autoroute_engine.autoroute_search_tree, p_autoroute_engine.generate_room_id_no(), test_level);
		...
		if (result instanceof CompleteFreeSpaceExpansionRoom) {
			calculate_target_doors((CompleteFreeSpaceExpansionRoom) result, room_neighbours.own_net_objects,
					p_autoroute_engine);
		}
		return result;
	}

	private static void calculate_incomplete_rooms_with_empty_neighbours(ObstacleExpansionRoom p_room,
		...
		for (int i = 0; i < room_shape.border_line_count(); ++i) {
			...
			if (SortedRoomNeighbours.insert_door_ok(p_room, curr_line)) {
				...
				FreeSpaceExpansionRoom new_room = p_autoroute_engine.add_incomplete_expansion_room(new_room_shape,
						p_room.get_layer(), new_contained_shape);
				...
				if (curr_object.contains_net(p_autoroute_engine.get_net_no())) {
					...
					if (curr_connection_shape != null && p_room.get_shape().intersects(curr_connection_shape)) {
						...
						TargetItemExpansionDoor new_target_door = new TargetItemExpansionDoor(curr_item,
								curr_entry.shape_index_in_object, p_room, p_autoroute_engine.autoroute_search_tree);
	...
	private static SortedRoomNeighbours calculate_neighbours(ExpansionRoom p_room, int p_net_no,
		...
		// Calculate the touching neigbour objects and sort them in counterclock
		// sence
		...
		for (ShapeTree.TreeEntry curr_entry : overlapping_objects) {
			...
			if ((p_room instanceof IncompleteFreeSpaceExpansionRoom) && !curr_object.is_trace_obstacle(p_net_no)) {
				// delay processing the target doors until the room shape will
				// not change any more
				result.own_net_objects.add(curr_entry);
				continue;
			}
			TileShape curr_shape = curr_object.get_tree_shape(p_autoroute_search_tree,
			...
			if (dimension > 1) {
				if (completed_room instanceof ObstacleExpansionRoom && curr_object instanceof Item) {
					...
					if (curr_item.is_route()) {
						...
						ObstacleExpansionRoom curr_overlap_room = item_info
								.get_expansion_room(curr_entry.shape_index_in_object, p_autoroute_search_tree);
			...
			if (dimension == 1) {
				...
				result.add_sorted_neighbour(curr_shape, intersection, touching_sides[0], touching_sides[1], false,
						false);
				...
				if (curr_object instanceof ExpansionRoom) {
					...
					if (curr_item.is_route()) {
						...
						ItemAutorouteInfo item_info = curr_item.get_autoroute_info();
						neighbour_room = item_info.get_expansion_room(curr_entry.shape_index_in_object,
			...
			} else // dimensin = 0
			{
				...
				if (neighbour_room_corner_no >= 0) {
					...
					// The previous border line is preferred to make the shape
					// of the incomplete room as big as possible
					...
					if (touching_side_no_of_neighbour_room < 0
							&& p_test_level.ordinal() >= TestLevel.CRITICAL_DEBUGGING_OUTPUT.ordinal()) {
						System.out.println(
								"AutorouteEngine.SortedRoomNeighbours.calculate: touching_side_no_of_neighbour_room >= 0 expected");
					}
				}
				result.add_sorted_neighbour(curr_shape, intersection, touching_side_no_of_room,
						touching_side_no_of_neighbour_room, room_touch_is_corner, neighbour_room_touch_is_corner);
			}
		}
		return result;
	}
	...
	private void add_sorted_neighbour(TileShape p_neighbour_shape, TileShape p_intersection,
			int p_touching_side_no_of_room, int p_touching_side_no_of_neighbour_room, boolean p_room_touch_is_corner,
		...
		SortedRoomNeighbour new_neighbour = new SortedRoomNeighbour(p_neighbour_shape, p_intersection,
				p_touching_side_no_of_room, p_touching_side_no_of_neighbour_room, p_room_touch_is_corner,
	...
	private boolean try_remove_edge(int p_net_no, ShapeSearchTree p_autoroute_search_tree, TestLevel p_test_level) {
		...
		if (remove_edge_no >= 0) {
			...
			IncompleteFreeSpaceExpansionRoom enlarged_room = new IncompleteFreeSpaceExpansionRoom(enlarged_shape,
					curr_incomplete_room.get_layer(), curr_incomplete_room.get_contained_shape());
			Collection<IncompleteFreeSpaceExpansionRoom> new_rooms = p_autoroute_search_tree
					.complete_shape(enlarged_room, p_net_no, null, null);
			if (new_rooms.size() != 1) {
	...
	public void calculate_new_incomplete_rooms(AutorouteEngine p_autoroute_engine) {
		...
		while (it.hasNext()) {
			...
			boolean intersection_with_prev_neighbour_ends_at_corner = (first_touching_side_no != last_touching_side_no
					|| prev_neighbour == this.sorted_neighbours.last())
					&& prev_neighbour.last_corner().equals(room_simplex.corner(curr_next_no));
			boolean intersection_with_next_neighbour_starts_at_corner = (first_touching_side_no != last_touching_side_no
					|| prev_neighbour == this.sorted_neighbours.last())
			...
			if (!neighbours_touch) {
				...
				// start_edge_line is only used for the first new incomplete
				// room.
				Line middle_edge_line = null;
				int curr_touching_side_no = last_touching_side_no;
				boolean first_time = true;
				// The loop goes backwards fromm the edge line of next_neigbour
				// to the edge line of prev_neigbour.
				for (;;) {
					...
					if (this.from_room instanceof IncompleteFreeSpaceExpansionRoom) {
						...
						if (curr_touching_side_no == last_touching_side_no
							...
							// neighbour to the first corner of the next
							// neighbour to cut off
							// the outstanding corners of the room shape in the
							// empty space.
							...
							if (incomplete_room.get_contained_shape().side_of(cut_line) != Side.ON_THE_LEFT) {
								// Otherwise p_room.contained_shape would no
								// longer be contained
					...
					boolean last_time = curr_touching_side_no == first_touching_side_no
							&& !(prev_neighbour == this.sorted_neighbours.last() && first_time)
							// The expression above handles the case, when all
							// neigbours are on 1 edge line.
							|| corner_cut_off;

					Line end_edge_line;
					// end_edge_line is only used for the last new incomplete
					// room.
					if (last_time) {
						...
						if (end_edge_line.direction().side_of(middle_line_dir) != Side.ON_THE_LEFT) {
							// Concave corner between the middle and the last
							// line.
					...
					if (!new_room_shape.is_empty()) {
						...
						if (!new_contained_shape.is_empty()) {
							FreeSpaceExpansionRoom new_room = p_autoroute_engine.add_incomplete_expansion_room(
	...
	static boolean insert_door_ok(ExpansionRoom p_room_1, ExpansionRoom p_room_2, TileShape p_door_shape) {
		...
		// Insert 1 dimensional doors of trace rooms only, if they are parallel
		// to the trace line.
		// Otherwise there may be check ripup problems with entering at the
			...
			if (Math.abs(delta_distance) <= c_dist_tolerance) {
				...
				if (this.first_corner().equals(p_other.first_corner())) {
					...
					if (Math.abs(delta_distance) <= c_dist_tolerance) {
						...
						// Otherwise there may be a short 1 dim. touch at a link
						// between 2 trace lines.
						// In this case equality is ok, because the 2
						// intersection pieces with
						// the expansion room are identical, so that only 1
						// obstacle is needed.
						{
							...
							Line this_compare_line = this.neighbour_shape
									.border_line(this.touching_side_no_of_neighbour_room);
							Line other_compare_line = p_other.neighbour_shape
									.border_line(p_other.touching_side_no_of_neighbour_room);
							delta_distance = compare_dir.compare_from(this_compare_line.direction(),
		...
		public Point first_corner() {
			if (precalculated_first_corner == null) {
				if (room_touch_is_corner) {
					...
					Point curr_first_corner = neighbour_shape
							.corner(neighbour_shape.next_no(touching_side_no_of_neighbour_room));
