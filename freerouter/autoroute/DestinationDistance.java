...
public class DestinationDistance {
	 ...
	 */
	public DestinationDistance(ExpansionCostFactor[] p_trace_costs, boolean[] p_layer_active,
			double p_min_normal_via_cost, double p_min_cheap_via_cost) {
		...
		// Note: for inner layers we assume, that cost in preferred direction is
		// 1
	...
	public double calculate(IntBox p_box, int p_layer) {
		...
		// calculate shortest distance to component side box
		{
			...
			if (!component_side_box_is_empty) {
				result = p_box.weighted_distance(component_side_box, trace_costs[0].horizontal,
						trace_costs[0].vertical);
			}
			...
			if (min_solder_side_trace_cost < min_component_side_trace_cost)
				tmp_distance = min_solder_side_trace_cost * solder_side_max_delta
						+ min_component_side_trace_cost * solder_side_min_delta + min_normal_via_cost;
			else
				tmp_distance = min_component_side_trace_cost * solder_side_max_delta
						+ min_solder_side_trace_cost * solder_side_min_delta + min_normal_via_cost;
			...
			tmp_distance = component_side_max_delta + component_side_min_delta * min_component_inner_trace_cost
			...
			tmp_distance = inner_side_max_delta + inner_side_min_delta * min_component_inner_trace_cost
					+ min_normal_via_cost;
			...
			tmp_distance = solder_side_max_delta + +min_component_solder_inner_trace_cost * solder_side_min_delta
					+ 2 * min_normal_via_cost;
			result = Math.min(result, tmp_distance);
		...
		// calculate shortest distance to solder side box
		{
			...
			if (!solder_side_box_is_empty) {
				result = p_box.weighted_distance(solder_side_box, trace_costs[p_layer].horizontal,
						trace_costs[p_layer].vertical);
			}

			// calculate two layer distance
			double tmp_distance;
			if (min_component_side_trace_cost < min_solder_side_trace_cost) {
				tmp_distance = min_component_side_trace_cost * component_side_max_delta
						+ min_solder_side_trace_cost * component_side_min_delta + min_normal_via_cost;
			} else {
				tmp_distance = min_solder_side_trace_cost * component_side_max_delta
						+ min_component_side_trace_cost * component_side_min_delta + min_normal_via_cost;
			}
			result = Math.min(result, tmp_distance);
			tmp_distance = solder_side_max_delta + solder_side_min_delta * min_solder_inner_trace_cost
					+ 2 * min_normal_via_cost;
			result = Math.min(result, tmp_distance);
			if (active_layer_count <= 2) {
				return result;
			}
			tmp_distance = inner_side_min_delta * min_solder_inner_trace_cost + inner_side_max_delta
					+ min_normal_via_cost;
			result = Math.min(result, tmp_distance);

			// calculate three layer distance

			tmp_distance = component_side_max_delta + min_component_solder_inner_trace_cost * component_side_min_delta
					+ 2 * min_normal_via_cost;
			result = Math.min(result, tmp_distance);
		...
		if (!inner_side_box_is_empty) {
			result = p_box.weighted_distance(inner_side_box, trace_costs[p_layer].horizontal,
					trace_costs[p_layer].vertical);
		}
