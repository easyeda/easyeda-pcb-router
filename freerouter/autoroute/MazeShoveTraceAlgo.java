...
public class MazeShoveTraceAlgo {
	...
	public static boolean check_shove_trace_line(MazeListElement p_list_element, ObstacleExpansionRoom p_obstacle_room,
		...
		// only traces with the same halfwidth and the same clearance class can
		// be shoved.
		...
		if (from_door.dimension == 2) {
			...
			if (shove_into_direction_of_trace_start) {
				...
				// shove from the endpoint to the start point of the line
				// segment
			...
			boolean section_ok;
			// shove only from the right most section to the right or from the
			// left most section to the left.
			...
			if (p_shove_to_the_left && !door_line_swapped || !p_shove_to_the_left && door_line_swapped) {
				section_ok = p_list_element.section_no_of_door == p_list_element.door.maze_search_element_count() - 1
						&& (p_list_element.shape_entry.a.distance_square(door_line_segment.b) <= check_dist_square
								|| p_list_element.shape_entry.b
										.distance_square(door_line_segment.b) <= check_dist_square);
			} else {
				section_ok = p_list_element.section_no_of_door == 0 && (p_list_element.shape_entry.a
						.distance_square(door_line_segment.a) <= check_dist_square
			...
			if (side_of_trace_line == Side.ON_THE_LEFT) {
				if (p_shove_to_the_left) {
					...
					shove_line_segment = new LineSegment(start_closing_line,
		...
		if (!segment_ist_point) {
			shove_width = ShoveTraceAlgo.check(p_board, shove_line_segment, p_shove_to_the_left, trace_layer,
		...
		// From_door_compare_distance is used to check, that a door is between
		// from_door and the end point
		// of the shove line.
		...
		for (ExpansionDoor curr_door : room_doors) {
			...
			if (curr_door.dimension == 2 && shove_width >= Integer.MAX_VALUE) {
				...
				// check, that curr_door is on the same border_line as
				// p_from_door.
				...
				if (curr_door_projection.distance(from_corner) + compensated_trace_half_width <= shove_width) {
					...
					for (int i = 0; i < line_sections.length; ++i) {
						...
						if (curr_line_section.a.distance_square(from_corner) <= curr_line_section.b
								.distance_square(from_corner)) {
