		...
		if (this.drills == null || p_autoroute_engine.get_net_no() != this.net_no) {
			...
			for (TreeEntry curr_entry : overlaps) {
				...
				if (!prev_obstacle_shape.contains(curr_obstacle_shape)) {
					// Checked to avoid multiple cutout for example for vias
					// with the same shape on all layers.
			...
			for (int i = 0; i < drill_shapes.length; ++i) {
				...
				if (p_attach_smd) {
					curr_drill_location = calc_pin_center_in_drill(curr_drill_shape, drill_first_layer,
							p_autoroute_engine.board);
					if (curr_drill_location == null) {
						curr_drill_location = calc_pin_center_in_drill(curr_drill_shape, drill_last_layer,
								p_autoroute_engine.board);
					}
				}
				if (curr_drill_location == null) {
				...
				ExpansionDrill new_drill = new ExpansionDrill(curr_drill_shape, curr_drill_location, drill_first_layer,
						drill_last_layer);
