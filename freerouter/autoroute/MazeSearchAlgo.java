...
public class MazeSearchAlgo {
	...
	MazeSearchAlgo(AutorouteEngine p_autoroute_engine, AutorouteControl p_ctrl) {
		...
		ctrl = p_ctrl;
		// To get reproducable random numbers in the ripup algorithm.
		random_generator.setSeed(p_ctrl.ripup_costs);
	...
	public boolean occupy_next_element() {
		...
		if (list_element.next_room != null) {
			if (!expand_to_room_doors(list_element)) {
				...
				// In case nothing was expanded allow the section to be occupied
				// from somewhere else, if the next room is thin.
			}
		}
	...
	private boolean expand_to_room_doors(MazeListElement p_list_element) {
		...
		int layer_no = p_list_element.next_room.get_layer();// 获取当前room的下一个room所在的层；

		boolean layer_active = ctrl.layer_active[layer_no];// 查询下一个room所在层是否可用；
		if (!layer_active) {
		...
		double half_width = ctrl.compensated_trace_half_width[layer_no];// 获取半线宽；
		boolean curr_door_is_small = false;
		if (p_list_element.door instanceof ExpansionDoor) {// 第一次的门是TargetItemExpansionDoor，并不会是扩展的门；
		...
		FloatPoint shape_entry_middle = p_list_element.shape_entry.a.middle_point(p_list_element.shape_entry.b);// 获取当前元素的中点

		if (this.ctrl.with_neckdown && p_list_element.door instanceof TargetItemExpansionDoor) {// 这一段，获取最小的线宽；
		...
		if (p_list_element.next_room instanceof ObstacleExpansionRoom) {
			...
			if (next_room_shape.min_width() < 2 * half_width) {
				next_room_is_thick = false; // to prevent probles with the
											// opposite side
			} else if (!p_list_element.already_checked && p_list_element.door.get_dimension() == 1
					&& !curr_door_is_small) {
				// The algorithm below works only, if p_location is on the
				// border of p_room_shape.
			...
			if (!p_list_element.already_checked) {
				if (curr_door_is_small) {
					...
					if (next_room_is_thick) {
						// check to enter the thick room from a ripped item
						// through a small door (after ripup)
						...
						if (!shove_trace_room(p_list_element, obstacle_room)) {
							if (ripup_costs > 0) {
								...
								MazeListElement new_element = new MazeListElement(p_list_element.door,
										...
										p_list_element.sorting_value + ripup_costs, p_list_element.next_room,
										p_list_element.shape_entry, true, p_list_element.adjustment, true);
		...
		for (ExpansionDoor to_door : p_list_element.next_room.get_doors()) {
			...
			if (expand_to_door(to_door, p_list_element, ripup_costs, next_room_is_thick,
		...
		if (ctrl.vias_allowed && !(p_list_element.door instanceof ExpansionDrill)) {// 非钻孔才能扩展；
			if ((something_expanded || next_room_is_thick)
					&& p_list_element.next_room instanceof CompleteFreeSpaceExpansionRoom) {
				// avoid setting something_expanded to true when next_room is
				// thin to allow occupying by different sections of the door
				Collection<DrillPage> overlapping_drill_pages = this.autoroute_engine.drill_page_array
						.overlapping_pages(p_list_element.next_room.get_shape());// 获取相交的drill page
				for (DrillPage to_drill_page : overlapping_drill_pages) {
					expand_to_drill_page(to_drill_page, p_list_element);// 将drill page 加入到element list中；
					something_expanded = true;
				}
			} else if (p_list_element.next_room instanceof ObstacleExpansionRoom) {
	...
	private boolean expand_to_target_doors(MazeListElement p_list_element, boolean p_next_room_is_thick,
		...
		if (p_curr_door_is_small) {
			...
			if (p_list_element.door instanceof ExpansionDoor) {
				CompleteExpansionRoom from_room = ((ExpansionDoor) p_list_element.door)
						.other_room(p_list_element.next_room);
				if (from_room instanceof ObstacleExpansionRoom) {
					// otherwise entering through the small door may fail,
		...
		for (TargetItemExpansionDoor to_door : p_list_element.next_room.get_target_doors()) {
			...
			if (!p_next_room_is_thick) {
				// check the line from p_shape_entry_middle to the nearest
				// point.
				...
				if (!check_points[0].equals(check_points[1])) {
					...
					boolean check_ok = autoroute_engine.board.check_forced_trace_polyline(check_polyline,
							ctrl.trace_half_width[curr_layer], curr_layer, curr_net_no_arr,
							ctrl.trace_clearance_class_no, ctrl.max_shove_trace_recursion_depth,
							ctrl.max_shove_via_recursion_depth, ctrl.max_spring_over_recursion_depth);
					if (!check_ok) {
						continue;
					}
				}
			...
			if (expand_to_door_section(to_door, 0, new_shape_entry, p_list_element, 0,
					MazeSearchElement.Adjustment.NONE)) {
				result = true;
			}
		}
		...
		for (int i = 0; i < line_sections.length; ++i) {
			...
			if (p_next_room_is_thick) {
				...
				if (p_to_door.dimension == 1 && line_sections.length == 1
					...
					// check entering the p_to_door at an acute corner of the
					// shape of p_list_element.next_room
				...
				// expand only doors on the opposite side of the room from the
				// shape_entry.
	...
	private boolean door_is_small(ExpansionDoor p_door, double p_trace_width) {
		if (p_door.dimension == 1 || p_door.first_room instanceof CompleteFreeSpaceExpansionRoom
			...
			if (door_shape.is_empty()) {
				if (this.autoroute_engine.board.get_test_level().ordinal() >= board.TestLevel.ALL_DEBUGGING_OUTPUT
						.ordinal()) {
	...
	private boolean expand_to_door_section(ExpandableObject p_door, int p_section_no, FloatLine p_shape_entry,
			MazeListElement p_from_element, int p_add_costs, MazeSearchElement.Adjustment p_adjustment) {
	...
	private void expand_to_drill(ExpansionDrill p_drill, MazeListElement p_from_element, int p_add_costs) {
		...
		if (p_from_element.door instanceof DrillPage
			...
			// If expansion comes from a pin with trace exit directions the
			// eapansion_value is calculated
			// from the nearest trace exit point instead from the center olf the
			// pin.
			Item from_item = ((TargetItemExpansionDoor) p_from_element.backtrack_door).item;
			if (from_item instanceof board.Pin) {
				FloatPoint nearest_exit_corner = ((board.Pin) from_item)
		...
		double expansion_value = p_from_element.expansion_value + p_add_costs + nearest_point.weighted_distance(
				compare_corner, ctrl.trace_costs[layer].horizontal, ctrl.trace_costs[layer].vertical);
	...
	private void expand_to_drill_page(DrillPage p_drill_page, MazeListElement p_from_element) {
		...
		int layer = p_from_element.next_room.get_layer();// 获取当前room坐在的层信息；
		FloatPoint from_element_shape_entry_middle = p_from_element.shape_entry.a
				.middle_point(p_from_element.shape_entry.b);// 获取中点
		FloatPoint nearest_point = p_drill_page.shape.nearest_point(from_element_shape_entry_middle);// 获取drill
																										// page离中点的最近点
		double expansion_value = p_from_element.expansion_value + ctrl.min_normal_via_cost;// 扩展后的成本
		double sorting_value = expansion_value + nearest_point.weighted_distance(from_element_shape_entry_middle,
				ctrl.trace_costs[layer].horizontal, ctrl.trace_costs[layer].vertical)
	...
	private void expand_to_drills_of_page(MazeListElement p_from_element) {
		...
		Collection<ExpansionDrill> drill_list = drill_page.get_drills(this.autoroute_engine,
				this.ctrl.attach_smd_allowed);
	...
	private void expand_to_other_layers(MazeListElement p_list_element) {
		...
		if (curr_drill.room_arr[p_list_element.section_no_of_door] instanceof ObstacleExpansionRoom) {
			...
			Item curr_obstacle_item = ((ObstacleExpansionRoom) curr_drill.room_arr[p_list_element.section_no_of_door])
					.get_item();
			if (!(curr_obstacle_item instanceof board.Via)) {
				return;
			}
			...
			for (;;) {
				...
				board.ForcedPadAlgo.CheckDrillResult drill_result = ForcedViaAlgo.check_layer(
						ctrl.via_radius_arr[curr_layer], ctrl.via_clearance_class, ctrl.attach_smd_allowed,
		...
		for (int to_layer = via_lower_bound; to_layer <= via_upper_bound; ++to_layer) {
			...
			MazeSearchElement curr_drill_layer_info = curr_drill
					.get_maze_search_element(to_layer - curr_drill.first_layer);
		...
		while (it.hasNext()) {
			...
			if (curr_item instanceof Connectable) {
				for (int i = 0; i < curr_item.tree_shape_count(search_tree); ++i) {
					TileShape contained_shape = ((Connectable) curr_item).get_trace_connection_shape(search_tree, i);// 获取了一个中心点，中心点构成的bbox
...
//		start_rooms中存储了起始pin的中心点，以及在不同层之间的信息；
		...
		if (this.autoroute_engine.maintain_database) {
			// add the completed start rooms carried over from the last
			// autoroute to the start rooms.
		...
		for (CompleteFreeSpaceExpansionRoom curr_room : completed_start_rooms) {
			...
			while (it2.hasNext()) {
				...
				if (curr_door.is_destination_door()) {
					continue;
				} // 这里是一个问题，如果这里将destination_door过滤掉，那么find——connec里的有个if条件，基本就是无用的了；
				...
				MazeListElement new_list_element = new MazeListElement(curr_door, 0, null, 0, 0, sorting_value,
						curr_room, shape_entry, false, MazeSearchElement.Adjustment.NONE, false);

//				public MazeListElement(ExpandableObject p_door, int p_section_no_of_door, ExpandableObject p_backtrack_door,
//						int p_section_no_of_backtrack_door, double p_expansion_value, double p_sorting_value,
//						CompleteExpansionRoom p_next_room, FloatLine p_shape_entry, boolean p_room_ripped,
//						MazeSearchElement.Adjustment p_adjustment, boolean p_already_checked) 
	...
	private int check_ripup(MazeListElement p_list_element, Item p_obstacle_item, boolean p_door_is_small) {
		...
		if (p_door_is_small) {
			// allow entering a via or trace, if its corresponding border
			// segment is smaller than the current trace width
		...
		if (room_was_shoved) {
			if (previous_item != null && previous_item != p_obstacle_item
		...
		double detour = 1;
		if (fanout_via_cost_factor <= 1) // p_obstacle_item does not belong to a
											// fanout
		{
	...
	private static double calc_fanout_via_ripup_cost_factor(board.Trace p_trace) {
		...
		for (int i = 0; i < 2; ++i) {
			...
			if (curr_trace_contact instanceof board.Pin
				...
				protect_fanout_via = true;
					...
					&& curr_trace_contact.get_fixed_state() == board.FixedState.SHOVE_FIXED) {
	...
	private boolean shove_trace_room(MazeListElement p_list_element, ObstacleExpansionRoom p_obstacle_room) {
		if (p_list_element.section_no_of_door != 0
			...
			// No delay of occupation necessesary because inner sections of a
	 ...
	 * connected to p_ignore_item are ignored in the check.
	 */
	private boolean enter_through_small_door(MazeListElement p_list_element, Item p_ignore_item) {
		...
		// create a perpendicular line segment of length 2 * check_radius
		// through the door center
