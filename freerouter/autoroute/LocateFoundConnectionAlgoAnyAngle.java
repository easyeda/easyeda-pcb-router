...
class LocateFoundConnectionAlgoAnyAngle extends LocateFoundConnectionAlgo {
	...
	protected LocateFoundConnectionAlgoAnyAngle(MazeSearchAlgo.Result p_maze_search_result, AutorouteControl p_ctrl,
			ShapeSearchTree p_search_tree, AngleRestriction p_angle_restriction, SortedSet<Item> p_ripped_item_list,
	...
	protected Collection<FloatPoint> calculate_next_trace_corners() {
		...
		if (this.current_to_door_index >= this.current_target_door_index) {
			if (this.current_to_door_index == this.current_target_door_index) {
				FloatPoint nearest_point = this.current_target_shape.nearest_point(this.current_from_point.round())
						.to_float();
		...
		if (this.current_from_point.side_of(door_left_corner, door_right_corner) != Side.ON_THE_RIGHT) {
			...
			if (this.current_from_point.scalar_product(this.previous_from_point, door_left_corner) >= 0) {
				...
				// That may not be the case if the door line is crossed almost
				// parallel.
		...
		// current_from_point
		// through the interval from left_most_visible_point to
		// right_most_visible_point,
		// by advancing the door index as far as possible, so that still
		// somthing is visible.
		...
		for (;;) {
			...
			if (left_tangent_point != null && right_tangent_point != null
				...
				// The gap between left_most_visible_point and
				// right_most_visible_point ist to small
				...
				if (left_corner_distance <= right_corner_distance) {
					...
					result_corner = left_turn_next_corner(this.current_from_point, trace_halfwidth_max,
							door_left_corner, door_right_corner);
				} else {
					new_door_ind = right_ind;
					result_corner = right_turn_next_corner(this.current_from_point, trace_halfwidth_max,
							door_right_corner, door_left_corner);
				}
				break;
			}
			...
			if (this.current_from_point.side_of(next_left_corner, next_right_corner) != Side.ON_THE_RIGHT) {
				...
				if (door_left_corner == null
						&& this.current_from_point.scalar_product(this.previous_from_point, next_left_corner) >= 0) {
					// Also the left corner of the door is passed.
					// That may not be the case if the door line is crossed
					// almost parallel.
					next_left_corner = null;
				}
				if (door_right_corner == null
						&& this.current_from_point.scalar_product(this.previous_from_point, next_right_corner) >= 0) {
					// Also the right corner of the door is passed.
					next_right_corner = null;
				}
				if (next_left_corner == null && next_right_corner == null) {
					// The door is completely passed.
					// Should not happen because the previous door was not
					// passed compledtely.
					if (this.test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
						System.out.println(
			...
			// even if all parameter points are defined
			{
				if (next_left_corner.side_of(this.current_from_point, door_right_corner) == Side.ON_THE_RIGHT) {
					...
					result_corner = right_turn_next_corner(this.current_from_point, trace_halfwidth_max,
							door_right_corner, next_left_corner);
					break;
				}
				...
				if (next_right_corner.side_of(this.current_from_point, door_left_corner) == Side.ON_THE_LEFT) {
					...
					result_corner = left_turn_next_corner(this.current_from_point, trace_halfwidth_max,
							door_left_corner, next_right_corner);
					break;
				}
			}
			boolean visability_range_gets_smaller_on_the_right_side = (door_right_corner == null);
			if (door_right_corner != null
					&& next_right_corner.side_of(this.current_from_point, door_right_corner) != Side.ON_THE_RIGHT) {
				FloatPoint curr_tangential_point = this.current_from_point.left_tangential_point(next_right_corner,
						trace_halfwidth_max);
			...
			if (door_left_corner != null
					&& next_left_corner.side_of(this.current_from_point, door_left_corner) != Side.ON_THE_LEFT) {
				FloatPoint curr_tangential_point = this.current_from_point.right_tangential_point(next_left_corner,
						trace_halfwidth_max);
				if (curr_tangential_point != null) {
					FloatLine check_line = new FloatLine(this.current_from_point, curr_tangential_point);
		...
		if (end_of_trace) {
			...
			if (left_tangent_point != null
					&& nearest_point.side_of(this.current_from_point, left_tangent_point) == Side.ON_THE_LEFT) {
				// The nearest target point is to the left of the visible range,
				// add another corner
				new_door_ind = left_ind + 1;
				FloatPoint target_right_corner = this.current_target_shape
						.corner_approx(this.current_target_shape.index_of_right_most_corner(this.current_from_point));
				FloatPoint curr_corner = right_left_tangential_point(this.current_from_point, target_right_corner,
				...
				if (curr_corner != null) {
					...
					&& nearest_point.side_of(this.current_from_point, right_tangent_point) == Side.ON_THE_RIGHT) {
				// The nearest target point is to the right of the visible
				// range, add another corner
				FloatPoint target_left_corner = this.current_target_shape
						.corner_approx(this.current_target_shape.index_of_left_most_corner(this.current_from_point));
				new_door_ind = right_ind + 1;
				FloatPoint curr_corner = left_right_tangential_point(this.current_from_point, target_left_corner,
						door_right_corner, trace_halfwidth_max);
				if (curr_corner != null) {
					result_corner = curr_corner;
					end_of_trace = false;
				}
		...
		for (int i = check_from_door_index; i < new_door_ind; ++i) {
			...
			if (Math.abs(curr_dist) < trace_halfwidth_middle) {
				FloatPoint curr_corrected_result = right_left_tangential_point(check_line.a, check_line.b,
						curr_left_corner, trace_halfwidth_max);
				if (curr_corrected_result != null) {
					if (corrected_result == null || curr_corrected_result.side_of(this.current_from_point,
							corrected_result) == Side.ON_THE_RIGHT) {
			...
			if (Math.abs(curr_dist) < trace_halfwidth_middle) {
				FloatPoint curr_corrected_result = left_right_tangential_point(check_line.a, check_line.b,
						curr_right_corner, trace_halfwidth_max);
				if (curr_corrected_result != null) {
					if (corrected_result == null || curr_corrected_result.side_of(this.current_from_point,
							corrected_result) == Side.ON_THE_LEFT) {
						corrected_door_ind = i;
						corrected_result = curr_corrected_result;
					}
				}
	 ...
	 */
	private FloatPoint right_turn_next_corner(FloatPoint p_from_corner, double p_dist, FloatPoint p_to_corner,
			FloatPoint p_next_corner) {
	 ...
	 */
	private FloatPoint left_turn_next_corner(FloatPoint p_from_corner, double p_dist, FloatPoint p_to_corner,
			FloatPoint p_next_corner) {
	 ...
	 */
	private FloatPoint right_left_tangential_point(FloatPoint p_from_point, FloatPoint p_to_point, FloatPoint p_center,
			double p_dist) {
	 ...
	 * radius p_dist. Returns the intersection of the 2 lines.
	 */
	private FloatPoint left_right_tangential_point(FloatPoint p_from_point, FloatPoint p_to_point, FloatPoint p_center,
			double p_dist) {
