...
public class LocateFoundConnectionAlgo45Degree extends LocateFoundConnectionAlgo {
	 ...
	 */
	public LocateFoundConnectionAlgo45Degree(MazeSearchAlgo.Result p_maze_search_result, AutorouteControl p_ctrl,
	...
	protected Collection<FloatPoint> calculate_next_trace_corners() {
		...
		int trace_halfwidth_add = trace_halfwidth + AutorouteEngine.TRACE_WIDTH_TOLERANCE; // add
																							// some
																							// tolerance
																							// for
																							// free
		...
		if (!shrinked_room_shape.is_empty()) {
			...
			boolean horizontal_first = calc_horizontal_first_from_door(curr_from_info.door, this.current_from_point,
					nearest_room_point);
			nearest_room_point = round_to_integer(nearest_room_point);
			result.add(calculate_additional_corner(this.current_from_point, nearest_room_point, horizontal_first,
		...
		if (this.current_to_door_index == this.current_target_door_index) {
			...
			FloatPoint add_corner = calculate_additional_corner(this.current_from_point, nearest_point, true,
					this.angle_restriction);
			if (!shrinked_room_shape.contains(add_corner)) {
				add_corner = calculate_additional_corner(this.current_from_point, nearest_point, false,
						this.angle_restriction);
			}
		...
		if (!(curr_to_info.door instanceof ExpansionDoor)) {
			System.out
					.println("LocateFoundConnectionAlgo45Degree.calculate_next_trace_corners: ExpansionDoor expected");
			return result;
		}
		...
		if (curr_to_door.dimension == 2) {
			// May not happen in free angle routing mode because then corners
			// are cut off.
			...
			if (curr_to_info.next_room != null) {
				...
				// with IntBox or IntOctagon the next calculation will not work,
				// because they have
			...
			if (!nearest_to_door_point_ok) {
				// may be the room has an acute (45 degree) angle at a corner of
				// the door
				nearest_to_door_point = curr_line_section.a.middle_point(curr_line_section.b);
			}
		}
		nearest_to_door_point = round_to_integer(nearest_to_door_point);
		boolean horizontal_first = calc_horizontal_first_to_door(curr_to_info.door, this.current_from_point,
				nearest_to_door_point);
		result.add(calculate_additional_corner(this.current_from_point, nearest_to_door_point, horizontal_first,
				this.angle_restriction));
	 ...
	 */
	private static boolean calc_horizontal_first_from_door(ExpandableObject p_from_door, FloatPoint p_from_point,
	 ...
	 */
	private boolean calc_horizontal_first_to_door(ExpandableObject p_to_door, FloatPoint p_from_point,
			FloatPoint p_to_point) {
