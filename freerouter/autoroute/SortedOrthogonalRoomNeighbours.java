...
public class SortedOrthogonalRoomNeighbours {
	...
	public static CompleteExpansionRoom calculate(ExpansionRoom p_room, AutorouteEngine p_autoroute_engine) {
		...
		SortedOrthogonalRoomNeighbours room_neighbours = SortedOrthogonalRoomNeighbours.calculate_neighbours(p_room,
				net_no, p_autoroute_engine.autoroute_search_tree, p_autoroute_engine.generate_room_id_no());
	...
	private static void calculate_incomplete_rooms_with_empty_neighbours(ObstacleExpansionRoom p_room,
		...
		if (!(room_shape instanceof IntBox)) {
			System.out.println(
					"SortedOrthoganelRoomNeighbours.calculate_incomplete_rooms_with_empty_neighbours: IntBox expected for room_shape");
			return;
		}
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (i == 0) {
				...
				System.out.println(
						"SortedOrthoganelRoomNeighbours.calculate_incomplete_rooms_with_empty_neighbours: illegal index i");
				return;
			}
			IntBox new_contained_box = room_box.intersection(new_room_box);
			FreeSpaceExpansionRoom new_room = p_autoroute_engine.add_incomplete_expansion_room(new_room_box,
					p_room.get_layer(), new_contained_box);
	...
	private static SortedOrthogonalRoomNeighbours calculate_neighbours(ExpansionRoom p_room, int p_net_no,
		...
		for (ShapeTree.TreeEntry curr_entry : overlapping_objects) {
			...
			if ((completed_room instanceof CompleteFreeSpaceExpansionRoom)
				...
				((CompleteFreeSpaceExpansionRoom) completed_room).calculate_target_doors(curr_entry, p_net_no,
			...
			if (!(curr_shape instanceof IntBox)) {
				System.out.println(
						"OrthogonalAutorouteEngine:calculate_sorted_neighbours: IntBox expected for curr_shape");
				return null;
			}
				...
				if (next_neighbour.first_touching_side == 0) {
					if (prev_neighbour.last_touching_side == 0) {
						if (prev_neighbour.intersection.ur.x < next_neighbour.intersection.ll.x) {
							insert_incomplete_room(p_autoroute_engine, prev_neighbour.intersection.ur.x,
									board_bounds.ll.y, next_neighbour.intersection.ll.x, this.room_shape.ll.y);
						}
					} else {
						if (prev_neighbour.intersection.ll.y > this.room_shape.ll.y
							...
							if (is_obstacle_expansion_room) {
								// no 2-dim doors between
								// obstacle_expansion_rooms and free space rooms
								// allowed.
					...
					if (prev_neighbour.last_touching_side == 1) {
						if (prev_neighbour.intersection.ur.y < next_neighbour.intersection.ll.y) {
							insert_incomplete_room(p_autoroute_engine, this.room_shape.ur.x,
									prev_neighbour.intersection.ur.y, board_bounds.ur.x,
						...
						if (prev_neighbour.intersection.ur.x < this.room_shape.ur.x
							...
							if (is_obstacle_expansion_room) {
								...
								if (prev_neighbour.last_touching_side == 0) {
									insert_incomplete_room(p_autoroute_engine, prev_neighbour.intersection.ur.x,
											board_bounds.ll.y, room_shape.ur.x, room_shape.ll.y);
								}
								...
								insert_incomplete_room(p_autoroute_engine, prev_neighbour.intersection.ur.x,
										board_bounds.ll.y, board_bounds.ur.x, next_neighbour.intersection.ll.y);
							}
						}
					}
					...
					if (prev_neighbour.last_touching_side == 2) {
						if (prev_neighbour.intersection.ll.x > next_neighbour.intersection.ur.x) {
							insert_incomplete_room(p_autoroute_engine, next_neighbour.intersection.ur.x,
									this.room_shape.ur.y, prev_neighbour.intersection.ll.x, board_bounds.ur.y);
						}
					} else {
						if (prev_neighbour.intersection.ur.y < this.room_shape.ur.y
							...
							if (is_obstacle_expansion_room) {
								...
								// allowed.
								if (prev_neighbour.last_touching_side == 1) {
									insert_incomplete_room(p_autoroute_engine, room_shape.ur.x,
											prev_neighbour.intersection.ur.y, board_bounds.ur.x, room_shape.ur.y);
								}
								insert_incomplete_room(p_autoroute_engine, next_neighbour.intersection.ur.x,
										room_shape.ur.y, room_shape.ur.x, board_bounds.ur.y);
							} else {
								insert_incomplete_room(p_autoroute_engine, next_neighbour.intersection.ur.x,
										prev_neighbour.intersection.ur.y, board_bounds.ur.x, board_bounds.ur.y);
							}
						}
					}
					...
					if (prev_neighbour.last_touching_side == 3) {
						if (prev_neighbour.intersection.ll.y > next_neighbour.intersection.ur.y) {
							insert_incomplete_room(p_autoroute_engine, board_bounds.ll.x,
									next_neighbour.intersection.ur.y, this.room_shape.ll.x,
						...
						if (next_neighbour.intersection.ur.y < this.room_shape.ur.y
							...
							if (is_obstacle_expansion_room) {
								...
								insert_incomplete_room(p_autoroute_engine, board_bounds.ll.x,
										next_neighbour.intersection.ur.y, room_shape.ll.x, room_shape.ur.y);
							} else {
								insert_incomplete_room(p_autoroute_engine, board_bounds.ll.x,
										next_neighbour.intersection.ur.y, prev_neighbour.intersection.ll.x,
										board_bounds.ur.y);
							}
						}
					}
	...
	private void insert_incomplete_room(AutorouteEngine p_autoroute_engine, int p_ll_x, int p_ll_y, int p_ur_x,
			int p_ur_y) {
	...
	private boolean try_remove_edge(int p_net_no, ShapeSearchTree p_autoroute_search_tree) {
		...
		if (remove_edge_no >= 0) {
			...
			for (ExpansionDoor curr_door : door_list) {
				...
				// CompleteFreeSpaceExpansionRooms
				// for the information in complete_shape about the objects to
				// ignore.
			...
			IncompleteFreeSpaceExpansionRoom enlarged_room = new IncompleteFreeSpaceExpansionRoom(enlarged_box,
			...
			Collection<IncompleteFreeSpaceExpansionRoom> new_rooms = p_autoroute_search_tree
					.complete_shape(enlarged_room, p_net_no, ignore_object, ignore_shape);
			if (new_rooms.size() == 1) {
	...
	private class SortedRoomNeighbour implements Comparable<SortedRoomNeighbour> {
		public SortedRoomNeighbour(IntBox p_neighbour_shape, IntBox p_intersection) {
			...
			if (p_intersection.ll.y == room_shape.ll.y && p_intersection.ur.x > room_shape.ll.x
					&& p_intersection.ll.x < room_shape.ur.x) {
				edge_interiour_touches_obstacle[0] = true;
			}
			if (p_intersection.ur.x == room_shape.ur.x && p_intersection.ur.y > room_shape.ll.y
					&& p_intersection.ll.y < room_shape.ur.y) {
				edge_interiour_touches_obstacle[1] = true;
			}
			if (p_intersection.ur.y == room_shape.ur.y && p_intersection.ur.x > room_shape.ll.x
					&& p_intersection.ll.x < room_shape.ur.x) {
				edge_interiour_touches_obstacle[2] = true;
			}
			if (p_intersection.ll.x == room_shape.ll.x && p_intersection.ur.y > room_shape.ll.y
					&& p_intersection.ll.y < room_shape.ur.y) {
		...
		public int compareTo(SortedRoomNeighbour p_other) {
			...
			if (cmp_value == 0) {
				// The first touching points of this neighbour and p_other with
				// the room shape are equal.
