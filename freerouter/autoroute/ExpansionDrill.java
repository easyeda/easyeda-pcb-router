...
public class ExpansionDrill implements ExpandableObject {
	...
	public boolean calculate_expansion_rooms(AutorouteEngine p_autoroute_engine) {
		...
		Collection<board.SearchTreeObject> overlaps = p_autoroute_engine.autoroute_search_tree
				.overlapping_objects(search_shape, -1);
		for (int i = this.first_layer; i <= this.last_layer; ++i) {
			...
			if (found_room == null) {
				...
				IncompleteFreeSpaceExpansionRoom new_incomplete_room = new IncompleteFreeSpaceExpansionRoom(null, i,
						search_shape);
				Collection<CompleteFreeSpaceExpansionRoom> new_rooms = p_autoroute_engine
						.complete_expansion_room(new_incomplete_room);
				if (new_rooms.size() != 1) {
					// the size may be 0 because of an obstacle in the
					// compensated tree at this.location
					return false;
				}
