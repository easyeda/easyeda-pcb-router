...
public class BoardHandling {
	...
	public void set_layer_visibility(int p_layer, double p_value) {
		if (p_layer >= 0 && p_layer < graphics_context.layer_count()) {
			...
			if (p_value == 0 && settings.layer == p_layer) {
				// change the current layer to the best visible layer, if it
				// becomes invisible;
	...
	/**
	 * Hoverlees hack. get incompletes & violations num.
	 * 
	 * @return
	 */
	...
	public int getVioLationNum() {
		...
		return new Integer((clearance_violations.list.size() + 1) / 2);
	}
	...
	public void create_board(IntBox p_bounding_box, LayerStructure p_layer_structure, PolylineShape[] p_outline_shapes,
			String p_outline_clearance_class_name, BoardRules p_rules, board.Communication p_board_communication,
		...
		// graphics_context = new GraphicsContext(p_bounding_box, panel_size,
		// p_layer_structure, this.locale);
	}
	...
	public void change_user_unit_factor(double p_new_factor) {
		...
		this.coordinate_transform = new CoordinateTransform(p_new_factor, old_transform.user_unit,
	...
	public void terminate() throws InterruptedException {
		if (this.interactive_action_thread != null) {
			// The left button is used to stop the interactive action
			// thread.
	...
	public void mouse_moved(Point2D p_point) {
		...
		if (interactive_state != null && graphics_context != null) {
			...
			// If a repaint is necessary, it should be done in the individual
			// mouse_moved
	 ...
	 * currupted.
	 */
	public DsnFile.ReadResult import_design(java.io.InputStream p_design, board.BoardObservers p_observers,
	...
	public void copy_selected_items(Point2D p_from_location) {
		...
		Collection<Item> item_list = curr_state.get_item_list();
		FloatPoint from_location = graphics_context.coordinate_transform.screen_to_board(p_from_location);
		InteractiveState new_state = CopyItemState.get_instance(from_location, item_list,
				interactive_state.return_state, this, logfile);
		set_interactive_state(new_state);
	}
	...
	public void start_polygonshape_item(Point2D p_point) {
		...
		set_interactive_state(
				PolygonShapeConstructionState.get_instance(location, this.interactive_state, this, logfile));
	}
	...
	public void set_interactive_state(InteractiveState p_state) {
		if (p_state != null && p_state != interactive_state) {
			...
			if (!this.board_is_read_only) {
				p_state.set_toolbar();
				// this.panel.board_frame.set_context_sensitive_help(this.panel,
				// p_state.get_help_id());
			}
		}
	}
	...
	public void updateCacheIfNeeded() {
		if (updateFlag.compareAndSet(true, false)) {
			synchronized (this) {
				inCompletesNum = getInCompletesNum();
				cache = new ByteArrayOutputStream();
				set_board_read_only(false);
				export_specctra_session_file("tmp", cache);
				set_board_read_only(true);
				this.notify();
			}
		}
	}
	...
	public RouterCache waitCurrentCache() throws InterruptedException {
		...
		synchronized (this) {
			...
			if (updateFlag.get()) {
				...
				return new RouterCache(inCompletesNum, cache.toString());
			}
		}
	}

	public int reduceOptTimes() {
		return optTimes.decrementAndGet();
	}
