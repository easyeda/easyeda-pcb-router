...
public class MoveItemState extends InteractiveState {
	...
	public static MoveItemState get_instance(FloatPoint p_location, Collection<Item> p_item_list,
		...
		java.util.ResourceBundle resources = java.util.ResourceBundle
				.getBundle("interactive.resources.InteractiveState", p_board_handling.get_locale());
		...
		for (Item curr_item : item_list) {
			if (curr_item.is_user_fixed()) {
				...
				// Check if the whole connected set is inside the selected
				// items,
				// and add the items of the connected set to the move list in
				// this case.
				// Conduction areas are ignored, because otherwise components
				// with
	...
	private MoveItemState(FloatPoint p_location, Set<Item> p_item_list, Set<Component> p_component_list,
			Component p_first_component, InteractiveState p_parent_state, BoardHandling p_board_handling,
			Logfile p_logfile) {
		...
		for (Item curr_item : p_item_list) {
			// Copy the items in p_item_list, because otherwise the undo
	...
	private Vector adjust_to_placement_grid(Vector p_vector) {
		...
		IntPoint rounded_component_location = new_component_location.to_float()
