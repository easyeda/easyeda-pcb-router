...
public class Route {
	...
	public boolean next_corner(FloatPoint p_corner) {
		...
		Point ok_point = board.insert_forced_trace_segment(prev_corner, curr_corner, pen_half_width_arr[layer], layer,
				net_no_arr, clearance_class, max_shove_trace_recursion_depth, max_shove_via_recursion_depth,
				max_spring_over_recursion_depth, trace_tidy_width, pull_tight_accuracy, !is_stitch_mode,
	...
	public boolean change_layer(int p_to_layer) {
		...
		for (int i = 0; i < this.via_rule.via_count(); ++i) {
			...
			result = board.forced_via(curr_via_info, this.prev_corner, this.net_no_arr, clearance_class,
					pen_half_width_arr, max_shove_trace_recursion_depth, 0, this.trace_tidy_width,
	...
	private boolean connect(Point p_from_point, IntPoint p_to_point) {
		...
		for (int i = 1; i < corners.length; ++i) {
			...
			while (!from_corner.equals(to_corner)) {
				Point curr_ok_point = board.insert_forced_trace_segment(from_corner, to_corner,
						pen_half_width_arr[layer], this.layer, net_no_arr, clearance_class,
	...
	public void draw(Graphics p_graphics, GraphicsContext p_graphics_context) {
		if (this.hilight_shove_failing_obstacle && this.shove_failing_obstacle != null) {
			this.shove_failing_obstacle.draw(p_graphics, p_graphics_context, p_graphics_context.get_violations_color(),
					1);
		}
		...
		if (nearest_target_point != null && prev_corner != null) {
			boolean curr_length_matching_ok = true; // used for drawing the
													// incomplete as violation
			...
			if (max_trace_length > 0 || min_trace_length > 0 && length_matching_color_intensity > 0) {
				...
				if (max_trace_length <= 0) {
					// max_trace_length not provided. Create an ellipse
					// containing the whole board.
				...
				if (incomplete_length < curr_max_trace_length && min_trace_length <= max_trace_length) {
					...
					// dist_focus_to_center^2 = bigger_radius^2 -
					// smaller_radius^2
					double smaller_radius = 0.5 * Math.sqrt(
							curr_max_trace_length * curr_max_trace_length - incomplete_length * incomplete_length);
					int ellipse_count;
					...
					if (ellipse_count == 2) {
						...
						smaller_radius = 0.5 * Math.sqrt(
								curr_min_trace_length * curr_min_trace_length - incomplete_length * incomplete_length);
			...
			java.awt.Color draw_color = p_graphics_context.get_incomplete_color();
			double draw_width = Math.min(this.board.communication.get_resolution(Unit.MIL), 100); // problem
																									// with
																									// low
																									// resolution
																									// on
			...
			if (this.nearest_target_item != null && !this.nearest_target_item.is_on_layer(this.layer)) {
				...
				NetIncompletes.draw_layer_change_marker(draw_points[0], 4 * pen_half_width_arr[0], p_graphics,
						p_graphics_context);
			}
		}
	...
	void calc_nearest_target_point(FloatPoint p_from_point) {
		...
		// join the graphics update box by the nearest item, so that the
		// incomplete
	...
	private Point try_neckdown_at_start(IntPoint p_to_corner) {
		...
		double curr_clearance = this.board.rules.clearance_matrix.value(this.clearance_class,
				start_pin.clearance_class_no(), this.layer);
		...
		TimeLimit time_limit = new TimeLimit(CHECK_FORCED_TRACE_TIME_LIMIT);
		Point ok_point = board.insert_forced_trace_segment(prev_corner, p_to_corner, neck_down_halfwidth, layer,
				net_no_arr, clearance_class, max_shove_trace_recursion_depth, max_shove_via_recursion_depth,
				max_spring_over_recursion_depth, trace_tidy_width, pull_tight_accuracy, !is_stitch_mode, time_limit);
		return ok_point;
	}
	...
	private Point try_neckdown_at_end(Point p_from_corner, Point p_to_corner) {
		...
		double curr_clearance = this.board.rules.clearance_matrix.value(this.clearance_class,
				target_pin.clearance_class_no(), this.layer);
	...
	private Collection<Item> target_traces_and_areas; // from traces and
														// conduction areas
