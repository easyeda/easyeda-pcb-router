...
public abstract class LogfileScope {
	...
	public static final LogfileScope SET_CLEARANCE_COMPENSATION = new SetClearanceCompensationScope(
			"set_clearance_compensation");
	public static final LogfileScope SET_DRAG_COMPONENTS_ENABLED = new SetDragComponentsEnabledScope(
			"set_drag_componente_enabled");
	public static final LogfileScope SET_LAYER = new SetLayerScope("set_layer");
	public static final LogfileScope SET_MANUAL_TRACE_CLEARANCE_CLASS = new SetManualTraceClearanceClassScope(
			"set_manual_trace_clearance_class");
	public static final LogfileScope SET_MANUAL_TRACE_HALF_WIDTH = new SetManualTraceHalfWidthScope(
			"set_manual_trace_half_width");
	public static final LogfileScope SET_MANUAL_TRACEWITH_SELECTION = new SetManualTraceWidthSelectionScope(
	...
	public static final LogfileScope SET_PULL_TIGHT_REGION_WIDTH = new SetPullTightRegionWidthScope(
	...
	public static final LogfileScope EXTEND_TO_WHOLE_CONNECTED_SETS = new ExtendToWholeConnectedSetsScope(
			"extend_to_whole_connected_sets");
	public static final LogfileScope EXTEND_TO_WHOLE_CONNECTIONS = new ExtendToWholeConnectionsScope(
			"extend_to_whole_connections");
	public static final LogfileScope EXTEND_TO_WHOLE_COMPONENTS = new ExtendToWholeComponentsScope(
	...
	public static final LogfileScope ASSIGN_SELECTED_TO_NEW_NET = new AssignSelectedToNewNetScope(
			"assign_selected_to_new_net");
	public static final LogfileScope ASSIGN_SELECTED_TO_NEW_GROUP = new AssignSelectedToNewGroupScope(
	 ...
	 */
	private static LogfileScope[] arr = { UNDO, REDO, GENERATE_SNAPSHOT, SET_CLEARANCE_COMPENSATION, SET_LAYER,
			SET_MANUAL_TRACE_CLEARANCE_CLASS, SET_MANUAL_TRACE_HALF_WIDTH, SET_MANUAL_TRACEWITH_SELECTION,
			SET_SNAP_ANGLE, SET_SELECTABLE, SET_SELECT_ON_ALL_LAYER, SET_STITCH_ROUTE, SET_TRACE_HALF_WIDTH,
			SET_PULL_TIGHT_REGION_WIDTH, SET_PULL_TIGHT_ACCURACY, SET_PUSH_ENABLED, SET_IGNORE_CONDUCTION, START_SELECT,
			TOGGLE_SELECT, SELECT_REGION, EXTEND_TO_WHOLE_CONNECTED_SETS, EXTEND_TO_WHOLE_CONNECTIONS,
			EXTEND_TO_WHOLE_COMPONENTS, EXTEND_TO_WHOLE_NETS, ASSIGN_SELECTED_TO_NEW_NET, ASSIGN_SELECTED_TO_NEW_GROUP,
			FIX_SELECTED_ITEMS, UNFIX_SELECTED_ITEMS, DELETE_SELECTED, CUTOUT_ROUTE, OPTIMIZE_SELECTED,
			AUTOROUTE_SELECTED, FANOUT_SELECTED, COMPLETE_SCOPE, CANCEL_SCOPE, CREATING_TILE, CREATING_CIRCLE,
			CREATING_POLYGONSHAPE, ADDING_HOLE, CREATING_TRACE, CHANGE_LAYER, DRAGGING_ITEMS, MAKING_SPACE,
			COPYING_ITEMS, MOVE_ITEMS, TURN_90_DEGREE, ROTATE, CHANGE_PLACEMENT_SIDE, SET_ZOOM_WITH_WHEEL,
	 ...
	 */
	public abstract InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state,
	...
	private static abstract class CornerlistScope extends LogfileScope {
		 ...
		 */
		@Override
		public InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state,
				BoardHandling p_board_handling) {
			FloatPoint location = p_logfile.read_corner();
			if (location == null) {
				return null;
			}
			InteractiveState interactive_state = this.start_scope(location, p_return_state, p_board_handling);
		 ...
		 */
		public abstract InteractiveState start_scope(FloatPoint p_location, InteractiveState p_return_state,
				BoardHandling p_board_handling);
	}
	...
	private static class CreateTraceScope extends CornerlistScope {
		...
		@Override
		public InteractiveState start_scope(FloatPoint p_location, InteractiveState p_return_state,
				BoardHandling p_board_handling) {
	...
	private static class DragItemScope extends CornerlistScope {
		...
		public InteractiveState start_scope(FloatPoint p_location, InteractiveState p_return_state,
				BoardHandling p_board_handling) {
			return DragState.get_instance(p_location, p_return_state, p_board_handling, null);
		}
	}
	...
	private static class CopyItemScope extends CornerlistScope {
		...
		public InteractiveState start_scope(FloatPoint p_location, InteractiveState p_return_state,
			...
			if (p_return_state instanceof SelectedItemState) {
				...
				result = CopyItemState.get_instance(p_location, item_list, p_return_state.return_state,
	...
	private static class MoveItemScope extends CornerlistScope {
		...
		public InteractiveState start_scope(FloatPoint p_location, InteractiveState p_return_state,
			...
			if (p_return_state instanceof SelectedItemState) {
				java.util.Collection<board.Item> item_list = ((SelectedItemState) p_return_state).get_item_list();
				result = MoveItemState.get_instance(p_location, item_list, p_return_state.return_state,
						p_board_handling, null);
			} else {
	...
	private static class SelectRegionScope extends LogfileScope {
		...
		public InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state,
			...
			InteractiveState curr_state = SelectItemsInRegionState.get_instance(lower_left, p_return_state,
					p_board_handling, null);
	...
	private static class CutoutRouteScope extends LogfileScope {
		...
		public InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state,
			...
			InteractiveState curr_state = CutoutRouteState.get_instance(item_list, lower_left,
					p_return_state.return_state, p_board_handling, null);
			FloatPoint upper_right = p_logfile.read_corner();
			if (upper_right == null) {
				// user may have cancelled the state after the first corner
				return curr_state;
	...
	private static class SetSnapAngleScope extends LogfileScope {
		...
		public InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state,
			...
			p_board_handling.get_routing_board().rules
					.set_trace_angle_restriction(board.AngleRestriction.arr[new_snap_angle_no]);
			return p_return_state;
		}
	}
	...
	private static class SetSelectableScope extends LogfileScope {
		...
		public InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state,
			...
			board.ItemSelectionFilter.SelectableChoices item_type = board.ItemSelectionFilter.SelectableChoices
					.values()[item_type_no];
			if (selection == 0) {
	...
	private static class ZoomFrameScope extends LogfileScope {
		...
		public InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state,
				BoardHandling p_board_handling) {
			java.awt.geom.Point2D lower_left = p_board_handling.graphics_context.coordinate_transform
					.board_to_screen(p_logfile.read_corner());
			java.awt.geom.Point2D upper_right = p_board_handling.graphics_context.coordinate_transform
					.board_to_screen(p_logfile.read_corner());
			return p_return_state;
		}
	}
