...
public class SelectedItemState extends InteractiveState {
	...
	private SelectedItemState(Set<Item> p_item_list, InteractiveState p_parent_state, BoardHandling p_board_handling,
			Logfile p_logfile) {
	...
	public InteractiveState key_typed(char p_key_char) {
		...
		if (p_key_char == 'a') {
			...
			result = MoveItemState.get_instance(hdlg.get_current_mouse_position(), item_list, this.return_state, hdlg,
					logfile);
	...
	public InteractiveState assign_items_to_new_group() {
		RoutingBoard board = hdlg.get_routing_board();
		board.generate_snapshot();
		// Take the gravity point of all item centers for the location of the
		// new component.
		double gravity_x = 0;
		double gravity_y = 0;
		int pin_count = 0;
		...
		for (int i = 0; i < pin_arr.length; ++i) {
			...
			board.insert_pin(new_component.no, i, net_no_arr, curr_via.clearance_class_no(),
	...
	public InteractiveState autoroute(Stoppable p_stoppable_thread) {
		...
		for (Item curr_item : autoroute_item_list) {
			...
			AutorouteEngine.AutorouteResult autoroute_result = hdlg.get_routing_board().autoroute(curr_item,
					hdlg.settings, via_costs, p_stoppable_thread, null);
	...
	public InteractiveState fanout(Stoppable p_stoppable_thread) {
		...
		for (Pin curr_pin : fanout_list) {
			...
			hdlg.get_routing_board().start_marking_changed_area();
			AutorouteEngine.AutorouteResult autoroute_result = hdlg.get_routing_board().fanout(curr_pin, hdlg.settings,
					-1, p_stoppable_thread, null);
			if (autoroute_result == AutorouteEngine.AutorouteResult.ROUTED) {
				++found_count;
			} else if (autoroute_result != AutorouteEngine.AutorouteResult.ALREADY_CONNECTED) {
				++not_found_count;
	...
	public InteractiveState pull_tight(Stoppable p_stoppable_thread) {
		...
		for (Item curr_item : item_list) {
			...
			if (curr_item instanceof PolylineTrace) {
				...
				OptViaAlgo.opt_via_location(hdlg.get_routing_board(), (Via) curr_item, null,
		...
		if (hdlg.settings.push_enabled && !interrupted) {
			hdlg.get_routing_board().opt_changed_area(new int[0], null, hdlg.settings.trace_pull_tight_accuracy, null,
