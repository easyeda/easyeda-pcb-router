...
public class RouteState extends InteractiveState {
	...
	public static RouteState get_instance(FloatPoint p_location, InteractiveState p_parent_state,
		...
		if (picked_item instanceof board.Pin && net_count > 1) {
			// tie pin, remove nets, which are already conneccted to this pin on
			// the current layer.
			route_net_no_arr = get_route_net_numbers_at_tie_pin((board.Pin) picked_item,
					p_board_handling.settings.layer);
		} else {
		...
		if (picked_item instanceof Trace) {
			...
			if (picked_corner instanceof IntPoint
				...
				if (picked_trace instanceof PolylineTrace) {
					FloatPoint nearest_point = ((PolylineTrace) picked_trace).polyline()
							.nearest_point_approx(p_location);
			...
			if (start_ok && !p_board_handling.settings.manual_rule_selection) {
				// Pick up the half with and the clearance class of the found
				// trace.
		...
		new_instance.route = new Route(location, p_board_handling.settings.layer, trace_half_widths, layer_active_arr,
				route_net_no_arr, trace_clearance_class, p_board_handling.get_via_rule(route_net_no_arr[0]),
				...
				routing_board, is_stitch_route, p_board_handling.settings.automatic_neckdown,
				p_board_handling.settings.via_snap_to_smd_center, p_board_handling.settings.hilight_routing_obstacle);
	...
	public boolean change_layer_action(int p_new_layer) {
		...
		if (p_new_layer >= 0 && p_new_layer < hdlg.get_routing_board().get_layer_count()) {
			...
			if (change_layer_succeeded) {
				...
				// check, if the layer change resulted in a connection to a
				// power plane.
				int old_layer = hdlg.settings.get_layer();
				ItemSelectionFilter selection_filter = new ItemSelectionFilter(
						ItemSelectionFilter.SelectableChoices.VIAS);
