...
public abstract class InteractiveActionThread extends Thread implements datastructures.Stoppable {
	...
	public static InteractiveActionThread get_read_logfile_instance(BoardHandling p_board_handling,
	...
	private static class ReadLogfileThread extends InteractiveActionThread {
		...
		protected void thread_action() {
			...
			java.util.ResourceBundle resources = java.util.ResourceBundle
					.getBundle("interactive.resources.InteractiveState", hdlg.get_locale());
			...
			while (!done) {
				...
				if (!done) {
					try {
						InteractiveState new_state = logfile_scope.read_scope(hdlg.logfile, hdlg.interactive_state,
								hdlg);
						if (new_state == null) {
