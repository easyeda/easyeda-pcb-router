...
public class CopyItemState extends InteractiveState {
	...
	public static CopyItemState get_instance(FloatPoint p_location, Collection<Item> p_item_list,
		...
		p_board_handling.remove_ratsnest(); // copying an item may change the
											// connectivity.
	...
	private CopyItemState(FloatPoint p_location, Collection<Item> p_item_list, InteractiveState p_parent_state,
	...
	public void insert() {
		...
		Map<Padstack, Padstack> padstack_pairs = new TreeMap<Padstack, Padstack>(); // Contains
																					// old
																					// and
																					// new
																					// padstacks
		...
		if (layer_changed) {
			...
			while (it.hasNext()) {
				...
				if (curr_ob instanceof Via) {
					...
					Padstack new_padstack = change_padstack_layers(curr_via.get_padstack(), current_layer, board,
							padstack_pairs);
					curr_via.set_padstack(new_padstack);
				}
			}
		}
		// Copy the components of the old items and assign the new items to the
		// copied components.
		...
		while (it.hasNext()) {
			...
			if (curr_cmp_no > 0) {
				...
				if (cmp_no_pairs.containsKey(curr_key)) {
					...
					Package new_package;
					if (layer_changed) {
						// create a new package with changed layers of the
						// padstacks.
						...
						for (int i = 0; i < new_pin_arr.length; ++i) {
							...
							Padstack new_padstack = change_padstack_layers(old_padstack, current_layer, board,
									padstack_pairs);
							new_pin_arr[i] = new Package.Pin(old_pin.name, new_padstack.no, old_pin.relative_location,
					...
					Component new_component = board.components.add(new_location, old_component.get_rotation_in_degree(),
	 ...
	 * p_new_layer.
	 */
	private static Padstack change_padstack_layers(Padstack p_old_padstack, int p_new_layer, RoutingBoard p_board,
