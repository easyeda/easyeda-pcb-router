...
public class TileConstructionState extends CornerItemConstructionState {
	...
	private void remove_concave_corners() {
		...
		while (new_length > 2) {
			...
			if (this.hdlg.get_routing_board().rules
					.get_trace_angle_restriction() != AngleRestriction.FORTYFIVE_DEGREE) {
				// skip concave corner
				corner_arr[new_length - 2] = last_corner;
			}
			--new_length;
			// In 45 degree case just skip last corner as nothing like the
			// following
			...
			if (this.hdlg.get_routing_board().rules.get_trace_angle_restriction() == AngleRestriction.NINETY_DEGREE) {
				// prevent generating a non orthogonal line by changing the
				// previous corner
