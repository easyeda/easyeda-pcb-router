...
public class ExpandTestState extends InteractiveState {
		...
		if (p_key_char == 'n') {
			if (in_autoroute) {
				...
				while (!completing_succeeded) {
					IncompleteFreeSpaceExpansionRoom next_room = this.autoroute_engine
							.get_first_incomplete_expansion_room();
					if (next_room == null) {
						break;
					}
	...
	private void init(FloatPoint p_location) {
		...
		if (route_item == null) {
			...
			IncompleteFreeSpaceExpansionRoom expansion_room = autoroute_engine.add_incomplete_expansion_room(null,
					layer, contained_shape);
	...
	private void complete_autoroute() {
		...
		if (search_result != null) {
			...
			this.autoroute_result = LocateFoundConnectionAlgo.get_instance(search_result, control_settings,
					...
					board.TestLevel.ALL_DEBUGGING_OUTPUT);
