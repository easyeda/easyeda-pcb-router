...
public class BatchAutorouterThread extends InteractiveActionThread {
	...
	protected void thread_action() {
		try {
			...
			boolean fanout_first = hdlg.settings.autoroute_settings.get_with_fanout()
			...
			if (hdlg.get_routing_board().rules.get_trace_angle_restriction() == board.AngleRestriction.FORTYFIVE_DEGREE
					&& hdlg.get_routing_board().get_test_level() != board.TestLevel.RELEASE_VERSION) {
				test.Validate.multiple_of_45_degree("after autoroute: ", hdlg.get_routing_board());
			}

			hdlg.isFinished = true;
		} catch (Exception e) {
			e.printStackTrace();
			hdlg.exception = e;
		}
