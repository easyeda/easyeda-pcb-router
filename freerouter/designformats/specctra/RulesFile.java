...
public class RulesFile {
	...
	public static void write(interactive.BoardHandling p_board_handling, java.io.OutputStream p_output_stream,
		...
		WriteScopeParameter write_scope_parameter = new WriteScopeParameter(routing_board,
				p_board_handling.settings.autoroute_settings, output_file,
		...
		for (;;) {
			...
			if (prev_token == Keyword.OPEN_BRACKET) {
				if (next_token == Keyword.RULE) {
					...
					Library.read_padstack_scope(scanner, layer_structure, coordinate_transform,
							routing_board.library.padstacks);
					...
					interactive.AutorouteSettings autoroute_settings = AutorouteSettings.read_scope(scanner,
							layer_structure);
	...
	private static void add_rules(java.util.Collection<Rule> p_rules, BasicBoard p_board, String p_layer_name) {
		...
		for (Rule curr_rule : p_rules) {
			if (curr_rule instanceof Rule.WidthRule) {
				...
				Structure.set_clearance_rule((Rule.ClearanceRule) curr_rule, layer_no, coordinate_transform,
						p_board.rules, string_quote);
			}
		}
	}
	...
	private static boolean read_net_class(Scanner p_scanner, LayerStructure p_layer_structure, BasicBoard p_board) {
		...
		Network.insert_net_class(curr_class, p_layer_structure, p_board, p_board.communication.coordinate_transform,
				false);
		return true;
	}
}
