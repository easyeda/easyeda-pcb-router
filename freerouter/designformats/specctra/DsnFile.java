...
public class DsnFile {
	...
	public static ReadResult read(java.io.InputStream p_input_stream, interactive.BoardHandling p_board_handling,
			board.BoardObservers p_observers, datastructures.IdNoGenerator p_item_id_no_generator,
		...
		for (int i = 0; i < 3; ++i) {
			...
			if (i == 0) {
				...
				scanner.yybegin(SpecctraFileScanner.NAME); // to overread the
															// name of the pcb
															// for i = 2
			}
			if (!keyword_ok) {
		...
		ReadScopeParameter read_scope_par = new ReadScopeParameter(scanner, p_board_handling, p_observers,
		...
		if (read_ok) {
			...
			if (read_scope_par.autoroute_settings == null) {
				// look for power planes with incorrect layer type and adjust
		...
		// tests.Validate.check("after reading dsn",
	 ...
	 */
	public static boolean write(interactive.BoardHandling p_board_handling, java.io.OutputStream p_file,
			String p_design_name, boolean p_compat_mode) {
	...
	private static void write_pcb_scope(interactive.BoardHandling p_board_handling, IndentFileWriter p_file,
			String p_design_name, boolean p_compat_mode) throws java.io.IOException {
		BasicBoard routing_board = p_board_handling.get_routing_board();
		WriteScopeParameter write_scope_parameter = new WriteScopeParameter(routing_board,
				p_board_handling.settings.autoroute_settings, p_file,
