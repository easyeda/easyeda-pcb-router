...
public class Component extends ScopeKeyword {
	...
	public static void write_scope(WriteScopeParameter p_par, board.Component p_component) throws java.io.IOException {
	...
	private static void write_keepout_infos(WriteScopeParameter p_par, board.Component p_component)
		...
		for (int j = 0; j < 3; ++j) {
			...
			for (int i = 0; i < curr_keepout_arr.length; ++i) {
				...
				String cl_class_name = p_par.board.rules.clearance_matrix
						.get_name(curr_obstacle_area.clearance_class_no());
				if (cl_class_name == null) {
	...
	private static board.ObstacleArea get_keepout(board.BasicBoard p_board, int p_component_no, String p_name) {
		java.util.Iterator<datastructures.UndoableObjects.UndoableObjectNode> it = p_board.item_list
				.start_read_object();
		for (;;) {
	...
	private static ComponentPlacement.ComponentLocation read_place_scope(Scanner p_scanner) {
		try {
			java.util.Map<String, ComponentPlacement.ItemClearanceInfo> pin_infos = new java.util.TreeMap<String, ComponentPlacement.ItemClearanceInfo>();
			java.util.Map<String, ComponentPlacement.ItemClearanceInfo> keepout_infos = new java.util.TreeMap<String, ComponentPlacement.ItemClearanceInfo>();
			java.util.Map<String, ComponentPlacement.ItemClearanceInfo> via_keepout_infos = new java.util.TreeMap<String, ComponentPlacement.ItemClearanceInfo>();
			java.util.Map<String, ComponentPlacement.ItemClearanceInfo> place_keepout_infos = new java.util.TreeMap<String, ComponentPlacement.ItemClearanceInfo>();
			Object next_token = p_scanner.next_token();
			if (!(next_token instanceof String)) {
			...
			ComponentPlacement.ComponentLocation result = new ComponentPlacement.ComponentLocation(name, location,
					is_front, rotation, position_fixed, pin_infos, keepout_infos, via_keepout_infos,
					place_keepout_infos);
			return result;
		} catch (java.io.IOException e) {
			System.out.println("Component.read_scope: IO error scanning file");
	...
	private static ComponentPlacement.ItemClearanceInfo read_item_clearance_info(Scanner p_scanner)
			throws java.io.IOException {
