...
public class PolygonPath extends Path {
	...
	public geometry.planar.Shape transform_to_board(CoordinateTransform p_coordinate_transform) {
		...
		if (corner_arr.length <= 2) {
			IntPoint[] rounded_corner_arr = new IntPoint[corner_arr.length + 1];
			for (int i = 0; i < corner_arr.length; ++i) {
				rounded_corner_arr[i] = corner_arr[i].round();
			}
			FloatPoint point = new FloatPoint(corner_arr[corner_arr.length - 1].x+500, corner_arr[corner_arr.length - 1].y+500);
			rounded_corner_arr[corner_arr.length] = point.round();
