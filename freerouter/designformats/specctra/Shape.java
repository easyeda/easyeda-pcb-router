...
public abstract class Shape {
	 ...
	 */
	public static final ReadAreaScopeResult read_area_scope(Scanner p_scanner, LayerStructure p_layer_structure,
	...
	public static Rectangle read_rectangle_scope(Scanner p_scanner, LayerStructure p_layer_structure) {
		try {
			...
			if (next_token == Keyword.PCB_SCOPE) {
				...
				if (layer_no < 0 || layer_no >= p_layer_structure.arr.length) {
					System.out.println(
							"Shape.read_rectangle_scope: layer name " + layer_name + " not found in layer structure ");
				} else {
	 ...
	 * of the area.
	 */
	public static geometry.planar.Area transform_area_to_board(Collection<Shape> p_area,
	 ...
	 */
	public static geometry.planar.Area transform_area_to_board_rel(Collection<Shape> p_area,
			CoordinateTransform p_coordinate_transform) {
		int hole_count = p_area.size() - 1;
		if (hole_count <= -1) {
