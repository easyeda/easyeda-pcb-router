...
public class SessionFile {
	...
	public static boolean write(BasicBoard p_board, java.io.OutputStream p_output_stream, String p_design_name) {
		...
		try {
			...
			IdentifierType identifier_type = new IdentifierType(reserved_chars,
					p_board.communication.specctra_parser_info.string_quote);
	...
	public static void write_placement(BasicBoard p_board, IdentifierType p_identifier_type,
		...
		for (int i = 1; i <= p_board.library.packages.count(); ++i) {
			write_components(p_board, p_identifier_type, p_coordinate_transform, p_file,
	 ...
	 */
	public static void write_components(BasicBoard p_board, IdentifierType p_identifier_type,
			CoordinateTransform p_coordinate_transform, IndentFileWriter p_file, library.Package p_package)
	...
	public static void write_component(BasicBoard p_board, IdentifierType p_identifier_type,
			CoordinateTransform p_coordinate_transform, IndentFileWriter p_file, board.Component p_component)
			throws java.io.IOException {
		p_file.new_line();
		p_file.write("(place ");
	...
	public static void write_was_is(BasicBoard p_board, IdentifierType p_identifier_type, IndentFileWriter p_file)
			throws java.io.IOException {
		p_file.start_scope();
		p_file.write("was_is");
		...
		for (board.Pin curr_pin : board_pins) {
			...
			if (curr_pin.get_changed_to() != curr_pin) {
				...
				if (swap_cmp != null) {
					...
					p_file.write("-");
					library.Package.Pin package_pin = swap_cmp.get_package()
							.get_pin(swapped_with.get_index_in_package());
					p_identifier_type.write(package_pin.name, p_file);
				} else {
					System.out.println("SessionFile.write_was_is: component not found");
				}
	...
	private static void write_routes(BasicBoard p_board, IdentifierType p_identifier_type,
			CoordinateTransform p_coordinate_transform, IndentFileWriter p_file) throws java.io.IOException {
		p_file.start_scope();
		p_file.write("routes ");
		Resolution.write_scope(p_file, p_board.communication);
	...
	private static void write_library(BasicBoard p_board, IdentifierType p_identifier_type,
			CoordinateTransform p_coordinate_transform, IndentFileWriter p_file) throws java.io.IOException {
		p_file.start_scope();
		p_file.write("library_out ");
		for (int i = 0; i < p_board.library.via_padstack_count(); ++i) {
			write_padstack(p_board.library.get_via_padstack(i), p_board, p_identifier_type, p_coordinate_transform,
					p_file);
		}
		p_file.end_scope();
	}

	private static void write_padstack(library.Padstack p_padstack, BasicBoard p_board,
			IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform, IndentFileWriter p_file)
			throws java.io.IOException {
	...
	private static void write_network(BasicBoard p_board, IdentifierType p_identifier_type,
			CoordinateTransform p_coordinate_transform, IndentFileWriter p_file) throws java.io.IOException {
		p_file.start_scope();
	...
	private static void write_net(int p_net_no, BasicBoard p_board, IdentifierType p_identifier_type,
			CoordinateTransform p_coordinate_transform, IndentFileWriter p_file) throws java.io.IOException {
		...
		while (it.hasNext()) {
			...
			if (is_wire) {
				...
				write_conduction_area((ConductionArea) curr_item, p_board, p_identifier_type, p_coordinate_transform,
						p_file);
			}
		}
		if (header_written) {
	...
	static private void write_fixed_state(IndentFileWriter p_file, board.FixedState p_fixed_state)
			throws java.io.IOException {
