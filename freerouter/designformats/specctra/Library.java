...
public class Library extends ScopeKeyword {
	...
	public boolean read_scope(ReadScopeParameter p_par) {
		...
		for (;;) {
			...
			if (prev_token == OPEN_BRACKET) {
				if (next_token == Keyword.PADSTACK) {
					if (!read_padstack_scope(p_par.scanner, p_par.layer_structure, p_par.coordinate_transform,
		...
		if (p_par.via_padstack_names != null) {
			...
			if (found_padstack_count != via_padstacks.length) {
				// Some via padstacks were not found in the padstacks scope of
				// the dsn-file.
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i < keepout_arr.length; ++i) {
				...
				geometry.planar.Area curr_area = Shape.transform_area_to_board_rel(curr_keepout.shape_list,
			...
			library.Package.Keepout[] place_keepout_arr = new library.Package.Keepout[curr_package.place_keepouts
					.size()];
			...
			board.library.packages.add(curr_package.name, pin_arr, outline_arr, keepout_arr, via_keepout_arr,
					place_keepout_arr, curr_package.is_front);
		}
		return true;
	}
	...
	public static void write_padstack_scope(WriteScopeParameter p_par, library.Padstack p_padstack)
	...
	private void generate_missing_keepout_names(String p_keepout_type,
			Collection<Shape.ReadAreaScopeResult> p_keepout_list) {
