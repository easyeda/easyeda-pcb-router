...
public class SessionToEagle extends javax.swing.JFrame {
	...
	public static boolean get_instance(java.io.InputStream p_session, java.io.OutputStream p_output_stream,
		...
		SessionToEagle new_instance = new SessionToEagle(scanner, file_writer, p_board, p_board.communication.unit,
				p_board.communication.resolution, board_scale_factor);
	...
	private boolean process_session_scope() throws java.io.IOException {
		...
		for (int i = 0; i < 3; ++i) {
			...
			if (i == 0) {
				...
				this.scanner.yybegin(SpecctraFileScanner.NAME); // to overread
																// the name of
																// the pcb for i
																// = 2
			}
		...
		// Write a bounding rectangle with GROUP (Min_X-1 Min_Y-1) (Max_X+1
		// Max_Y+1);
		...
		for (;;) {
			...
			if (prev_token == Keyword.OPEN_BRACKET) {
				if (next_token == Keyword.ROUTES) {
					...
					// overread all scopes except the routes scope for the time
					// being
	...
	private boolean process_via_scope(String p_net_name) throws java.io.IOException {
		...
		// The Padstack name is of the form
		// Name$drill_diameter$from_layer-to_layer
	...
	private void write_pin_swap(board.Pin p_pin_1, board.Pin p_pin_2) throws java.io.IOException {
		...
		double[] location_1 = this.board.communication.coordinate_transform
				.board_to_dsn(p_pin_1.get_center().to_float());
		double[] location_2 = this.board.communication.coordinate_transform
				.board_to_dsn(p_pin_2.get_center().to_float());
