...
public class Package {
	...
	public Package(String p_name, PinInfo[] p_pin_info_arr, Collection<Shape> p_outline,
			Collection<Shape.ReadAreaScopeResult> p_keepouts, Collection<Shape.ReadAreaScopeResult> p_via_keepouts,
			Collection<Shape.ReadAreaScopeResult> p_place_keepouts, boolean p_is_front) {
		name = p_name;
			...
			for (;;) {
				...
				if (prev_token == Keyword.OPEN_BRACKET) {
					if (next_token == Keyword.PIN) {
						...
						Shape.ReadAreaScopeResult keepout_area = Shape.read_area_scope(p_scanner, p_layer_structure,
								false);
	...
	private static PinInfo read_pin_info(Scanner p_scanner) {
		try {
			...
			p_scanner.yybegin(SpecctraFileScanner.NAME); // to be able to handle
															// pin names
															// starting with a
															// digit.
			next_token = p_scanner.next_token();
