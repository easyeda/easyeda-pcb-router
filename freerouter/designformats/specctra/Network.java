...
public class Network extends ScopeKeyword {
		...
		for (;;) {
			...
			if (prev_token == OPEN_BRACKET) {
				if (next_token == Keyword.NET) {
					...
					rules.ViaInfo curr_via_info = read_via_info(p_par.scanner,
					...
					Collection<String> curr_via_rule = read_via_rule(p_par.scanner,
							p_par.board_handling.get_routing_board());
	...
	public static void write_via_infos(rules.BoardRules p_rules, IndentFileWriter p_file,
	...
	public static void write_via_rules(rules.BoardRules p_rules, IndentFileWriter p_file,
			IdentifierType p_identifier_type) throws java.io.IOException {
	...
	public static void write_net_classes(WriteScopeParameter p_par) throws java.io.IOException {
	...
	public static void write_net_class(rules.NetClass p_net_class, WriteScopeParameter p_par)
		...
		Rule.write_item_clearance_class(
				p_par.board.rules.clearance_matrix.get_name(p_net_class.get_trace_clearance_class()), p_par.file,
			...
			for (;;) {
				...
				if (prev_token == OPEN_BRACKET) {
					if (next_token == Keyword.PINS) {
						...
						calc_fixed(p_scanner);
						p_par.position_fixed.put(net_name, true);
					} else {
						skip_scope(p_scanner);
					}
				}
			...
			if (!net_rules.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					if (curr_ob instanceof Rule.WidthRule) {
						...
						rules.NetClass net_rule = p_board.rules.net_classes.find(trace_halfwidth,
								default_net_rule.get_trace_clearance_class(), default_net_rule.get_via_rule());
						if (net_rule == null) {
	...
	static rules.ViaInfo read_via_info(Scanner p_scanner, board.BasicBoard p_board) {
		try {
			...
			if (via_padstack == null) {
				// The padstack may not yet be inserted into the list of via
				// padstacks
			...
			if (clearance_class < 0) {
				// Clearance class not stored, because it is identical to the
				// default clearance class.
	...
	private static void insert_via_infos(Collection<rules.ViaInfo> p_via_infos, RoutingBoard p_board,
		...
		} else // no via infos found, create default via infos from the via
				// padstacks.
		{
	...
	private static void create_default_via_infos(board.BasicBoard p_board, rules.NetClass p_net_class,
			boolean p_attach_allowed) {
		...
		for (int i = 0; i < p_board.library.via_padstack_count(); ++i) {
			...
			rules.ViaInfo found_via_info = new rules.ViaInfo(via_name, curr_padstack, cl_class, attach_allowed,
					p_board.rules);
	...
	/**
	 * Inserts a via rule into the board. Replaces an already existing via rule with
	 * the same
	 */
	...
	private static void insert_net_classes(Collection<NetClass> p_net_classes, ReadScopeParameter p_par) {
		...
		for (NetClass curr_class : p_net_classes) {
			insert_net_class(curr_class, p_par.layer_structure, routing_board, p_par.coordinate_transform,
					p_par.via_at_smd_allowed);
		}
	}

	static void insert_net_class(NetClass p_class, LayerStructure p_layer_structure, board.BasicBoard p_board,
			CoordinateTransform p_coordinate_transform, boolean p_via_at_smd_allowed) {
		...
		for (Rule curr_rule : p_class.rules) {
			if (curr_rule instanceof Rule.WidthRule) {
				int trace_halfwidth = (int) Math
						.round(p_coordinate_transform.dsn_to_board(((Rule.WidthRule) curr_rule).value / 2));
				board_net_class.set_trace_half_width(trace_halfwidth);
			} else if (curr_rule instanceof Rule.ClearanceRule) {
				add_clearance_rule(p_board.rules.clearance_matrix, board_net_class, (Rule.ClearanceRule) curr_rule, -1,
				...
				for (Rule curr_rule : curr_layer_rule.rules) {
					if (curr_rule instanceof Rule.WidthRule) {
						...
						add_clearance_rule(p_board.rules.clearance_matrix, board_net_class,
								(Rule.ClearanceRule) curr_rule, layer_no, p_coordinate_transform);
						clearance_rule_found = true;
					} else {
	...
	static private void insert_class_pair_info(NetClass.ClassClass p_class_class, rules.NetClass p_first_class,
			rules.NetClass p_second_class, board.BasicBoard p_board, CoordinateTransform p_coordinate_transform) {
			...
			while (it.hasNext()) {
				...
				for (int i = 0; i < 2; ++i) {
					...
					if (p_layer_no < 0) {
						...
						p_clearance_matrix.set_value(curr_first_class_no, curr_second_class_no, p_layer_no,
								curr_clearance);
						p_clearance_matrix.set_value(curr_second_class_no, curr_first_class_no, p_layer_no,
								curr_clearance);
					}
				}
			}
	...
	private static void create_via_rule(Collection<String> p_use_via, rules.NetClass p_net_class,
			board.BasicBoard p_board, boolean p_attach_allowed) {
		rules.ViaRule new_via_rule = new rules.ViaRule(p_net_class.get_name());
		int default_via_cl_class = p_net_class.default_item_clearance_classes
				.get(rules.DefaultItemClearanceClasses.ItemClass.VIA);
	...
	private static void create_active_trace_layers(Collection<String> p_use_layer, LayerStructure p_layer_structure,
			rules.NetClass p_net_class) {
	...
	private static void add_clearance_rule(rules.ClearanceMatrix p_clearance_matrix, rules.NetClass p_net_class,
		...
		if (class_no < 0) {
			...
			// set the clearance values of the new class to the maximum of
			// curr_clearance and the
			// the existing values.
	...
	/**
	 * Gets the number of the clearance class with name combined of p_net_class_name
	 * and p_item_class_name. Creates a new class, if that class is not yet
	 * existing.
	 */
	static private int get_clearance_class(rules.ClearanceMatrix p_clearance_matrix, rules.NetClass p_net_class,
		...
		// initalise the clearance values of p_new_class_name from
		// p_net_class_name
		for (int i = 1; i < p_clearance_matrix.get_class_count(); ++i) {
	...
	/**
	 * Create the part library on the board. Can be called after the components are
	 * inserted. Returns false, if an error occured.
	 */
	private static boolean insert_logical_parts(ReadScopeParameter p_par) {
		...
		for (PartLibrary.LogicalPart next_part : p_par.logical_parts) {
			library.Package lib_package = search_lib_package(next_part.name, p_par.logical_part_mappings,
					routing_board);
			if (lib_package == null) {
				return false;
			}
			library.LogicalPart.PartPin[] board_part_pins = new library.LogicalPart.PartPin[next_part.part_pins.size()];
			int curr_index = 0;
			for (PartLibrary.PartPin curr_part_pin : next_part.part_pins) {
				...
				board_part_pins[curr_index] = new library.LogicalPart.PartPin(pin_no, curr_part_pin.pin_name,
						curr_part_pin.gate_name, curr_part_pin.gate_swap_code, curr_part_pin.gate_pin_name,
	...
	private static void insert_component(ComponentPlacement.ComponentLocation p_location, String p_lib_key,
		...
		board.Component new_component = routing_board.components.add(p_location.name, component_location,
				rotation_in_degree, p_location.is_front, curr_front_package, curr_back_package,
		...
		for (int i = 0; i < curr_package.pin_count(); ++i) {
			...
			if (pin_nets.size() == 0 && curr_padstack.getShapeSize() > 1) {
				fixed_state = board.FixedState.SYSTEM_FIXED;
			} else if (p_location.position_fixed) {
				fixed_state = board.FixedState.SYSTEM_FIXED;
			} else {
				fixed_state = board.FixedState.UNFIXED;
			}
			for (Net curr_pin_net : pin_nets) {
				rules.Net curr_board_net = routing_board.rules.nets.get(curr_pin_net.id.name,
						curr_pin_net.id.subnet_number);
				...
				if (p_par.position_fixed.containsKey(curr_board_net.name)
						&& p_par.position_fixed.get(curr_board_net.name)) {
			...
			if (clearance_class < 0) {
				if (curr_padstack.from_layer() == curr_padstack.to_layer()) {
					clearance_class = net_class.default_item_clearance_classes
							.get(rules.DefaultItemClearanceClasses.ItemClass.SMD);
				} else {
					clearance_class = net_class.default_item_clearance_classes
							.get(rules.DefaultItemClearanceClasses.ItemClass.PIN);
				}
			}
		...
		for (int k = 0; k <= 2; ++k) {
			...
			for (int i = 0; i < keepout_arr.length; ++i) {
				...
				int clearance_class = routing_board.rules.get_default_net_class().default_item_clearance_classes
				...
				if (keepout_info != null) {
					int curr_clearance_class = routing_board.rules.clearance_matrix
