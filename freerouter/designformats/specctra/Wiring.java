...
class Wiring extends ScopeKeyword {
	...
	private static void write_conduction_area_scope(WriteScopeParameter p_par, board.ConductionArea p_conduction_area)
		...
		Rule.write_item_clearance_class(
				p_par.board.rules.clearance_matrix.get_name(p_conduction_area.clearance_class_no()), p_par.file,
				p_par.identifier_type);
		p_par.file.end_scope();
	}

	static private void write_net(rules.Net p_net, IndentFileWriter p_file, IdentifierType p_identifier_type)
			throws java.io.IOException {
		p_file.new_line();
		p_file.write("(");
	...
	static private void write_fixed_state(IndentFileWriter p_file, FixedState p_fixed_state)
			throws java.io.IOException {
	...
	private Item read_wire_scope(ReadScopeParameter p_par) {
		...
		if (border_shape != null) {
			if (clearance_class_no < 0) {
				clearance_class_no = net_class.default_item_clearance_classes
			...
			geometry.planar.Area conduction_area = Shape.transform_area_to_board(area, p_par.coordinate_transform);
			result = board.insert_conduction_area(conduction_area, layer_no, net_no_arr, clearance_class_no, false,
					fixed);
		} else if (path instanceof PolygonPath) {
			if (clearance_class_no < 0) {
				clearance_class_no = net_class.default_item_clearance_classes
						.get(rules.DefaultItemClearanceClasses.ItemClass.TRACE);
			}
//			IntPoint[] corner_arr = new IntPoint[path.coordinate_arr.length / 2];
			ArrayList<IntPoint> corners = new ArrayList<>();
			double[] curr_point = new double[2];
			int length = path.coordinate_arr.length / 2;
...
//					return null;
					...
					corners.add(curr_corner.round());
				}
//				corners.add(curr_corner.round());
//				corner_arr[i] = curr_corner.round();
			}
			IntPoint[] corner_arr = corners.toArray(new IntPoint[corners.size()]);
			Polyline trace_polyline = new Polyline(corner_arr);
			// Traces are not yet normalized here because cycles may be removed
			// premature.
			result = board.insert_trace_without_cleaning(trace_polyline, layer_no, half_width, net_no_arr,
					clearance_class_no, fixed);
	...
	private static boolean via_exists(IntPoint p_location, library.Padstack p_padstack, int[] p_net_no_arr,
