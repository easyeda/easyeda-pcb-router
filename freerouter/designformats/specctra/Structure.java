...
class Structure extends ScopeKeyword {
	...
	public boolean read_scope(ReadScopeParameter p_par) {
			...
			if (prev_token == OPEN_BRACKET) {
				if (next_token == Keyword.BOUNDARY) {
					...
					if (p_par.layer_structure != null) {
						// correct the layer_structure because another layer isr
						// read
		...
		while (it.hasNext()) {
			...
			geometry.planar.Area plane_area = Shape.transform_area_to_board(plane_info.area.shape_list,
	...
	public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException {
		...
		for (;;) {
			...
			if (curr_keepout.get_component_no() != 0) {
				// keepouts belonging to a component are not written
				// individually.
				continue;
			}
	...
	static void write_default_rules(WriteScopeParameter p_par) throws java.io.IOException {
		...
		for (int i = 0; i < p_par.board.layer_structure.arr.length; ++i) {
			boolean write_layer_rule = p_par.board.rules.get_default_net_class()
					.get_trace_half_width(i) != p_par.board.rules.get_default_net_class().get_trace_half_width(0)
	...
	private static void write_control_scope(rules.BoardRules p_rules, IndentFileWriter p_file)
	...
	private static void write_keepout_scope(WriteScopeParameter p_par, board.ObstacleArea p_keepout)
			throws java.io.IOException {
	...
	static boolean read_layer_scope(Scanner p_scanner, BoardConstructionInfo p_board_construction_info,
		...
		try {
			...
			if (layer_ok) {
				Layer curr_layer = new Layer(layer_string, p_board_construction_info.found_layer_count, is_signal,
						net_names);
	...
	static void write_snap_angle(IndentFileWriter p_file, board.AngleRestriction p_angle_restriction)
			throws java.io.IOException {
		p_file.start_scope();
	...
	private boolean create_board(ReadScopeParameter p_par, BoardConstructionInfo p_board_construction_info) {
		...
		// Calculate an appropritate scaling between dsn coordinates and board
		// coordinates.
		...
		board.Communication.SpecctraParserInfo specctra_parser_info = new board.Communication.SpecctraParserInfo(
				p_par.string_quote, p_par.host_cad, p_par.host_version, p_par.constants, p_par.write_resolution,
				p_par.dsn_file_generated_by_host);
		board.Communication board_communication = new board.Communication(p_par.unit, p_par.resolution,
				specctra_parser_info, p_par.coordinate_transform, p_par.item_id_no_generator, p_par.observers);
		...
		int hole_cl_class_no = 0;

		if (board_rules != null) {
			if (p_board_construction_info.outline_clearance_class_name != null && board_rules.clearance_matrix != null) {
				hole_cl_class_no = board_rules.clearance_matrix.get_no(p_board_construction_info.outline_clearance_class_name);
				hole_cl_class_no = Math.max(hole_cl_class_no, 0);
			} else {
				hole_cl_class_no = board_rules.get_default_net_class().default_item_clearance_classes
						.get(rules.DefaultItemClearanceClasses.ItemClass.AREA);
			}
		}
		for (PolylineShape curr_outline_hole : hole_shapes) {
			for (int i = 0; i < board_layer_structure.arr.length; ++i) {
				board.insert_obstacle(curr_outline_hole, i, hole_cl_class_no, FixedState.SYSTEM_FIXED);
			}
		}
		...
		while (it3.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if (curr_ob instanceof Rule.WidthRule) {
					...
					Rule.ClearanceRule curr_rule = (Rule.ClearanceRule) curr_ob;
					set_clearance_rule(curr_rule, layer_no, p_par.coordinate_transform, p_board_rules,
							p_par.string_quote);
				}
			}
		}
		...
		while (it.hasNext()) {
			...
			if (curr_string.startsWith(p_string_quote)) {
				...
				if (curr_pair.length != 2) {
					// pairs with more than 1 underline like smd_via_same_net
					// are not implemented
					continue;
				}
			}
	...
	private static boolean insert_keepout(Shape.ReadAreaScopeResult p_area, ReadScopeParameter p_par,
		...
		geometry.planar.Area keepout_area = Shape.transform_area_to_board(p_area.shape_list,
				p_par.coordinate_transform);
		...
		if (curr_layer == Layer.SIGNAL) {
			...
			insert_keepout(board, keepout_area, curr_layer.no, p_area.clearance_class_name, p_keepout_type,
					p_fixed_state);
		} else {
