...
public class IntPoint extends Point implements java.io.Serializable {
	...
	/**
	 * Calculates the nearest point to this point on the horizontal or vertical line
	 * through p_other (Snaps this point to on ortogonal line through p_other).
	 */
	...
	/**
	 * Calculates the nearest point to this point on an orthogonal or diagonal line
	 * through p_other (Snaps this point to on 45 degree line through p_other).
	 */
	...
	/**
	 * Calculates a corner point p so that the lines through this point and p and
	 * from p to p_to_point are multiples of 45 degree, and that the angle at p will
	 * be 45 degree. If p_left_turn, p_to_point will be on the left of the line from
	 * this point to p, else on the right. Returns null, if the line from this point
	 * to p_to_point is already a multiple of 45 degree.
	 */
	public IntPoint fortyfive_degree_corner(IntPoint p_to_point, boolean p_left_turn) {
		...
		if (dy > 0 && dy < dx) {
			...
			// the line from this point to p_to_point is already a multiple of
			// 45 degree
			result = null;
		}
		return result;
	...
	/**
	 * Calculates a corner point p so that the lines through this point and p and
	 * from p to p_to_point are hprizontal or vertical, and that the angle at p will
	 * be 90 degree. If p_left_turn, p_to_point will be on the left of the line from
	 * this point to p, else on the right. Returns null, if the line from this point
	 * to p_to_point is already orthogonal.
	 */
