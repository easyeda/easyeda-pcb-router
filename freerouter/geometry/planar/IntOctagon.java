...
public class IntOctagon extends RegularTileShape implements java.io.Serializable {
	 ...
	 */
	public static final IntOctagon EMPTY = new IntOctagon(Limits.CRIT_INT, Limits.CRIT_INT, -Limits.CRIT_INT,
			-Limits.CRIT_INT, Limits.CRIT_INT, -Limits.CRIT_INT, Limits.CRIT_INT, -Limits.CRIT_INT);

	/**
	 * Creates an IntOctagon from 8 integer values. p_lx is the smallest x value of
	 * the shape. p_ly is the smallest y value of the shape. p_rx is the biggest x
	 * valuje af the shape. p_uy is the biggest y value of the shape. p_ulx is the
	 * intersection of the upper left diagonal boundary line with the x axis. p_lrx
	 * is the intersection of the lower right diagonal boundary line with the x
	 * axis. p_llx is the intersection of the lower left diagonal boundary line with
	 * the x axis. p_urx is the intersection of the upper right diagonal boundary
	...
	public IntOctagon translate_by(Vector p_rel_coor) {
		...
		return new IntOctagon(lx + rel_coor.x, ly + rel_coor.y, rx + rel_coor.x, uy + rel_coor.y,
				ulx + rel_coor.x - rel_coor.y, lrx + rel_coor.x - rel_coor.y, llx + rel_coor.x + rel_coor.y,
	...
	public IntOctagon offset(double p_distance) {
		...
		IntOctagon result = new IntOctagon(lx - width, ly - width, rx + width, uy + width, ulx - dia_width,
				lrx + dia_width, llx - dia_width, urx + dia_width);
	...
	public boolean is_normalized() {
		...
		boolean result = lx == on.lx && ly == on.ly && rx == on.rx && uy == on.uy && llx == on.llx && lrx == on.lrx
				&& ulx == on.ulx && urx == on.urx;
		return result;
	}
	 ...
	 * parameter type FloatPoint, the function may not be exact close to the border.
	 */
	@Override
	 ...
	 * p_border_line_no. The border lines are located in counterclock sense around
	 * this octagon.
	 */
	...
	public IntOctagon intersection(IntOctagon p_other) {
		IntOctagon result = new IntOctagon(Math.max(lx, p_other.lx), Math.max(ly, p_other.ly), Math.min(rx, p_other.rx),
				Math.min(uy, p_other.uy), Math.max(ulx, p_other.ulx), Math.min(lrx, p_other.lrx),
	...
	public boolean is_contained_in(IntOctagon p_other) {
		boolean result = lx >= p_other.lx && ly >= p_other.ly && rx <= p_other.rx && uy <= p_other.uy
				&& llx >= p_other.llx && ulx >= p_other.ulx && lrx <= p_other.lrx && urx <= p_other.urx;
	...
	public IntOctagon union(IntOctagon p_other) {
		IntOctagon result = new IntOctagon(Math.min(lx, p_other.lx), Math.min(ly, p_other.ly), Math.max(rx, p_other.rx),
				Math.max(uy, p_other.uy), Math.min(ulx, p_other.ulx), Math.max(lrx, p_other.lrx),
	...
	/**
	 * Returns true, if this octagon intersects with p_other and the intersection is
	 * 2-dimensional.
	 */
	...
	/**
	 * Calculates the border point of this octagon from p_point into the 45 degree
	 * direction p_dir. If this border point is not an IntPoint, the nearest outside
	 * IntPoint of the octagon is returned.
	 */
	 ...
	 * this octagon in the 45-degree directions. p_point is assumed to be located in
	 * the interiour of this octagon.
	 */
	...
	Side border_line_side_of(FloatPoint p_point, int p_line_no, double p_tolerance) {
		...
		if (p_line_no == 0) {
			...
			if (tmp > p_tolerance)
			// the p_point is above the the lower right border line of this
			// octagon
			{
				result = Side.ON_THE_RIGHT;
			} else if (tmp < -p_tolerance)
			// the p_point is below the the lower right border line of this
			// octagon
			{
				result = Side.ON_THE_LEFT;
			} else {
			...
			if (tmp < -p_tolerance) {
				// the p_point is below the the upper right border line of this
				// octagon
				result = Side.ON_THE_RIGHT;
			} else if (tmp > p_tolerance) {
				// the p_point is above the the upper right border line of this
				// octagon
				result = Side.ON_THE_LEFT;
			} else {
				result = Side.COLLINEAR;
			...
			if (tmp < -p_tolerance)
			// the p_point is below the the upper left border line of this
			// octagon
			{
				result = Side.ON_THE_RIGHT;
			} else if (tmp > p_tolerance)
			// the p_point is above the the upper left border line of this
			// octagon
			{
				result = Side.ON_THE_LEFT;
			} else {
			...
			if (tmp > p_tolerance) {
				// the p_point is above the the lower left border line of this
				// octagon
				result = Side.ON_THE_RIGHT;
			} else if (tmp < -p_tolerance) {
				// the p_point is below the the lower left border line of this
				// octagon
				result = Side.ON_THE_LEFT;
			} else {
				result = Side.COLLINEAR;
	...
	/**
	 * Divide p_d minus this octagon into 8 convex pieces, from which 4 have cut off
	 * a corner.
	 */
	@Override
	IntOctagon[] cutout_from(IntBox p_d) {
		...
		IntOctagon curr_oct = new IntOctagon(p_d.ll.x, boxes[0].ur.y, boxes[3].ll.x, p_d.ur.y, -Limits.CRIT_INT, c.ulx,
				-Limits.CRIT_INT, Limits.CRIT_INT);
		octagons[0] = curr_oct.normalize();

		// construct lower left octagon

		curr_oct = new IntOctagon(p_d.ll.x, p_d.ll.y, boxes[2].ll.x, boxes[0].ll.y, -Limits.CRIT_INT, Limits.CRIT_INT,
				-Limits.CRIT_INT, c.llx);
		octagons[1] = curr_oct.normalize();

		// construct lower right octagon

		curr_oct = new IntOctagon(boxes[2].ur.x, p_d.ll.y, p_d.ur.x, boxes[1].ll.y, c.lrx, Limits.CRIT_INT,
				-Limits.CRIT_INT, Limits.CRIT_INT);
		octagons[2] = curr_oct.normalize();

		// construct upper right octagon

		curr_oct = new IntOctagon(boxes[3].ur.x, boxes[1].ur.y, p_d.ur.x, p_d.ur.y, -Limits.CRIT_INT, Limits.CRIT_INT,
	...
	/**
	 * Divide p_divide_octagon minus cut_octagon into 8 convex pieces without sharp
	 * angles.
	 */
	@Override
	IntOctagon[] cutout_from(IntOctagon p_d) {
		...
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_1.rx - curr_1.left_x_value(curr_1.uy) > curr_2.upper_y_value(curr_1.rx) - curr_2.ly) {
			// switch the horizontal upper left divide line to vertical
			curr_1 = new IntOctagon(Math.min(curr_1.lx, curr_2.lx), curr_1.ly, curr_1.rx, curr_2.uy, curr_2.ulx,
					curr_1.lrx, curr_1.llx, curr_2.urx);

			curr_2 = new IntOctagon(curr_1.rx, curr_2.ly, curr_2.rx, curr_2.uy, curr_2.ulx, curr_2.lrx, curr_2.llx,
					curr_2.urx);
		...
		curr_2 = result[6];
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_2.upper_y_value(curr_1.rx) - curr_2.ly > curr_1.rx - curr_1.left_x_value(curr_2.ly))
		// switch the vertical upper left divide line to horizontal
		{
			curr_2 = new IntOctagon(curr_1.lx, curr_2.ly, curr_2.rx, Math.max(curr_2.uy, curr_1.uy), curr_1.ulx,
					curr_2.lrx, curr_1.llx, curr_2.urx);

			curr_1 = new IntOctagon(curr_1.lx, curr_1.ly, curr_1.rx, curr_2.ly, curr_1.ulx, curr_1.lrx, curr_1.llx,
					curr_1.urx);
		...
		curr_2 = result[5];
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_2.upper_y_value(curr_1.rx) - curr_1.ly > curr_2.right_x_value(curr_1.ly) - curr_2.lx)
		// switch the vertical upper right divide line to horizontal
		{
			curr_1 = new IntOctagon(curr_1.lx, curr_1.ly, curr_2.rx, Math.max(curr_2.uy, curr_1.uy), curr_1.ulx,
					curr_2.lrx, curr_1.llx, curr_2.urx);

			curr_2 = new IntOctagon(curr_2.lx, curr_2.ly, curr_2.rx, curr_1.ly, curr_2.ulx, curr_2.lrx, curr_2.llx,
					curr_2.urx);
		...
		curr_2 = result[4];
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_2.right_x_value(curr_2.uy) - curr_2.lx > curr_1.upper_y_value(curr_2.lx) - curr_2.uy)
		// switch the horizontal upper right divide line to vertical
		{
			curr_2 = new IntOctagon(curr_2.lx, curr_2.ly, Math.max(curr_2.rx, curr_1.rx), curr_1.uy, curr_1.ulx,
					curr_2.lrx, curr_2.llx, curr_1.urx);

			curr_1 = new IntOctagon(curr_1.lx, curr_1.ly, curr_2.lx, curr_1.uy, curr_1.ulx, curr_1.lrx, curr_1.llx,
					curr_1.urx);
		...
		curr_2 = result[3];
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_1.right_x_value(curr_1.ly) - curr_1.lx > curr_1.ly - curr_2.lower_y_value(curr_1.lx))
		// switch the horizontal lower right divide line to vertical
		{
			curr_1 = new IntOctagon(curr_1.lx, curr_2.ly, Math.max(curr_2.rx, curr_1.rx), curr_1.uy, curr_1.ulx,
					curr_2.lrx, curr_2.llx, curr_1.urx);

			curr_2 = new IntOctagon(curr_2.lx, curr_2.ly, curr_1.lx, curr_2.uy, curr_2.ulx, curr_2.lrx, curr_2.llx,
					curr_2.urx);
		...
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_2.uy - curr_2.lower_y_value(curr_2.rx) > curr_1.right_x_value(curr_2.uy) - curr_2.rx)
		// switch the vertical lower right divide line to horizontal
		{
			curr_2 = new IntOctagon(curr_2.lx, Math.min(curr_1.ly, curr_2.ly), curr_1.rx, curr_2.uy, curr_2.ulx,
					curr_1.lrx, curr_2.llx, curr_1.urx);

			curr_1 = new IntOctagon(curr_1.lx, curr_2.uy, curr_1.rx, curr_1.uy, curr_1.ulx, curr_1.lrx, curr_1.llx,
					curr_1.urx);
		...
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_1.uy - curr_1.lower_y_value(curr_1.lx) > curr_1.lx - curr_2.left_x_value(curr_1.uy))
		// switch the vertical lower left divide line to horizontal
		{
			curr_1 = new IntOctagon(curr_2.lx, Math.min(curr_1.ly, curr_2.ly), curr_1.rx, curr_1.uy, curr_2.ulx,
					curr_1.lrx, curr_2.llx, curr_1.urx);

			curr_2 = new IntOctagon(curr_2.lx, curr_1.uy, curr_2.rx, curr_2.uy, curr_2.ulx, curr_2.lrx, curr_2.llx,
					curr_2.urx);
		...
		if (!(curr_1.is_empty() || curr_2.is_empty())
				&& curr_2.rx - curr_2.left_x_value(curr_2.ly) > curr_2.ly - curr_1.lower_y_value(curr_2.rx))
		// switch the horizontal lower left divide line to vertical
		{
			curr_2 = new IntOctagon(Math.min(curr_2.lx, curr_1.lx), curr_1.ly, curr_2.rx, curr_2.uy, curr_2.ulx,
					curr_1.lrx, curr_1.llx, curr_2.urx);

			curr_1 = new IntOctagon(curr_2.rx, curr_1.ly, curr_1.rx, curr_1.uy, curr_1.ulx, curr_1.lrx, curr_1.llx,
					curr_1.urx);
	...
	public String toString() {
		String string = "lx :" + lx + "," + "ly :" + ly + "," + "rx :" + rx + "," + "uy :" + uy + "," + "ulx :" + ulx
				+ "," + "lrx :" + lrx + "," + "llx :" + llx + "," + "urx :" + urx;
		return string;
	}
