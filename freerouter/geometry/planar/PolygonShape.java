...
public class PolygonShape extends PolylineShape {
	...
	public PolygonShape(Polygon p_polygon) {
		...
		if (last_corner_no >= 2) {
			last_point_collinear = curr_corners[last_corner_no].side_of(curr_corners[last_corner_no - 1],
					curr_corners[0]) == Side.COLLINEAR;
		}
		...
		if (last_corner_no - first_corner_no >= 2) {
			first_point_collinear = curr_corners[0].side_of(curr_corners[1],
					curr_corners[last_corner_no]) == Side.COLLINEAR;
		}
	...
	public boolean contains_on_border(Point p_point) {
		// System.out.println("PolygonShape.contains_on_edge not yet
		// implemented");
		return false;
	}
			...
			for (int i = 0; i < loop_end; ++i) {
				...
				// try a horizontal division
				{
					...
					if (concave_corner.y >= min_y && concave_corner.y <= max_y) {
						...
						// Make shure, that the new shape will not be concave at
						// the projection point.
						// That might happen, if the boundary curve runs back in
						// itself.
				...
				// try a vertical division
				{
					...
					if (concave_corner.x >= min_x && concave_corner.x <= max_x) {
						...
						// make shure, that the new shape will be convex at the
						// projection point
						boolean projection_ok = curr_dist < min_projection_dist
