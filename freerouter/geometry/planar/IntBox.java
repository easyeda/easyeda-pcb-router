...
public class IntBox extends RegularTileShape implements java.io.Serializable {
	 ...
	 * this box. p_point is assumed to be located in the interiour of this nox. The
	 * funtion is only imoplemented for p_max_result_points <= 2;
	 */
	...
	/**
	 * Enlarges the box by p_offset. Contrary to the offset() method the result is
	 * an IntOctagon, not an IntBox.
	 */
	@Override
	...
	/**
	 * Returns the box, where the vertical boundary is offseted by p_dist. If p_dist
	...
	/**
	 * Shrinks the width and height of the box by the input width. The box will not
	 * vanish completely.
	 */
	...
	public Simplex to_Simplex() {
		...
		if (is_empty()) {
			...
			line_arr = new Line[4];
			line_arr[0] = Line.get_instance(ll, Direction.RIGHT);
			line_arr[1] = Line.get_instance(ur, Direction.UP);
			line_arr[2] = Line.get_instance(ur, Direction.LEFT);
			line_arr[3] = Line.get_instance(ll, Direction.DOWN);
		}
	...
	public boolean is_contained_in(IntBox p_other) {
		...
		if (ll.x < p_other.ll.x || ll.y < p_other.ll.y || ur.x > p_other.ur.x || ur.y > p_other.ur.y) {
			return false;
		}
		return true;
	}
	...
	public boolean contains_in_interiour(IntBox p_other) {
		...
		if (p_other.ll.x <= ll.x || p_other.ll.y <= ll.y || p_other.ur.x >= ur.x || p_other.ur.y >= ur.y) {
			return false;
		}
		return true;
	}
