...
public abstract class Point implements java.io.Serializable {
	...
	public static Point get_instance(int p_x, int p_y) {
		...
		if (Math.abs(p_x) > Limits.CRIT_INT || Math.abs(p_x) > Limits.CRIT_INT) {
	 ...
	 * line from p_1 to p_2; Side.ON_THE_RIGHT, if this Point is on the right of the
	 * line from p_1 to p_2; and Side.COLLINEAR, if this Point is collinear with p_1
	 * and p_2.
	 */
	...
	/**
	 * Calculates the perpendicular direction froma this point to p_line. Returns
	 * Direction.NULL, if this point lies on p_line.
	 */
	public Direction perpendicular_direction(Line p_line) {
		Side side = this.side_of(p_line);
		if (Math.sin(p_line.direction().angle_approx()) < 0) {
			side = side.negate();
		}
	...
	/**
	 * Returns 1, if this Point has a strict bigger x coordinate than p_other, 0, if
	 * the x cooordinates are equal, and -1 otherwise.
	 */
	public abstract int compare_x(Point p_other);

	/**
	 * Returns 1, if this Point has a strict bigger y coordinate than p_other, 0, if
	 * the y cooordinates are equal, and -1 otherwise.
	 */
	public abstract int compare_y(Point p_other);

	/**
	 * The function returns compare_x (p_other), if the result is not 0. Otherwise
	 * it returns compare_y (p_other).
	 */
