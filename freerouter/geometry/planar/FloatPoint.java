...
public class FloatPoint implements java.io.Serializable {
	...
	/**
	 * rounds the coordinates from an object of class Point_double to an object of
	 * class IntPoint
	 */
	...
	/**
	 * Rounds this point, so that if this point is on the right side of any directed
	 * line with direction p_dir, the result point will also be on the right side.
	 */
	...
	/**
	 * Rounds this point, so that if this point is on the left side of any directed
	 * line with direction p_dir, the result point will also be on the left side.
	 */
	...
	/**
	 * Returns an approximation of the perpendicular projection of this point onto
	 * p_line
	 */
	...
	/**
	 * Approximates a FloatPoint on the line from zero to this point with distance
	 * p_new_length from zero.
	 */
	...
	/**
	 * The function returns Side.ON_THE_LEFT, if this Point is on the left of the
	 * line from p_1 to p_2; and Side.ON_THE_RIGHT, if this Point is on the right of
	 * the line from p_1 to p_2. Collinearity is not defined, becouse numerical
	 * calculations ar not exact for FloatPoints.
	 */
	...
	/**
	 * Checks, if this point is contained in the box spanned by p_1 and p_2 with the
	 * input tolerance.
	 */
	 ...
	 * around p_to_point with radius p_distance. Solves the quadratic equation,
	 * which results by substituting x by the term in y from the equation of the
	 * polar line of a circle with center p_to_point and radius p_distance and
	 * putting it into the circle equation. The polar line is the line through the 2
	 * tangential points of the circle looked at from from this point and has the
	 * equation (this.x - p_to_point.x) * (x - p_to_point.x) + (this.y -
	 * p_to_point.y) * (y - p_to_point.y) = p_distance **2
	 */
	...
	/**
	 * Calculates the left tangential point of the line from this point to a circle
	 * around p_to_point with radius p_distance. Returns null, if this point is
	 * inside this circle.
	 */
	...
	/**
	 * Calculates the right tangential point of the line from this point to a circle
	 * around p_to_point with radius p_distance. Returns null, if this point is
	 * inside this circle.
	 */
	 ...
	 * calculating the intersection of the two lines perpendicular to and passing
	 * through the midpoints of the lines (this, p_1) and (p_1, p_2).
	 */
	public FloatPoint circle_center(FloatPoint p_1, FloatPoint p_2) {
		...
		double x_center = (slope_1 * slope_2 * (this.y - p_2.y) + slope_2 * (this.x + p_1.x)
				- slope_1 * (p_1.x + p_2.x)) / (2 * (slope_2 - slope_1));
		double y_center = (0.5 * (this.x + p_1.x) - x_center) / slope_1 + 0.5 * (this.y + p_1.y);
		return new FloatPoint(x_center, y_center);
	}

	/**
	 * Returns true, if this point is contained in the circle through p_1, p_2 and
	 * p_3.
	 */
	public boolean inside_circle(FloatPoint p_1, FloatPoint p_2, FloatPoint p_3) {
		...
		return (this.distance_square(center) < radius_square - 1); // - 1 is a
																	// tolerance
																	// for
																	// numerical
																	// stability.
	...
	public static IntOctagon bounding_octagon(FloatPoint[] p_point_arr) {
		...
		IntOctagon surrounding_octagon = new IntOctagon((int) Math.floor(lx), (int) Math.floor(ly), (int) Math.ceil(rx),
				(int) Math.ceil(uy), (int) Math.floor(ulx), (int) Math.ceil(lrx), (int) Math.floor(llx),
				(int) Math.ceil(urx));
