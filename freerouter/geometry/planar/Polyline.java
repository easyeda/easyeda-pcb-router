...
public class Polyline implements java.io.Serializable {
	 ...
	 * the i+1-th lines of the new created p_polyline for 0 <= i <
	...
	public FloatPoint nearest_point_approx(FloatPoint p_from_point) {
		...
		for (int i = 1; i < arr.length - 1; ++i) {
			...
			if (curr_distance < min_distance) {
				...
				if (projection.distance(corners[i]) + projection.distance(corners[i - 1]) < segment_length
						+ c_tolerance) {
					min_distance = curr_distance;
	 ...
	 * this polyline, if this polyline and p_other have no common end corner. If
	 * there is something to combine at the start of this polyline, p_other is
	 * inserted in front of this polyline. If there is something to combine at the
	 ...
	 * of the second split piece. p_endline and the line with number p_line_no must
	 * not be parallel. The order of the lines ins the two result pieces is
	 * preserved. p_line_no must be bigger than 0 and less then arr.length - 1.
	...
	public Polyline[] split(int p_line_no, Line p_end_line) {
		...
		if (this.corner(p_line_no).equals(new_end_corner)) {
			// skip line segment of length 0 at the beginning of the second
			// piece
	 ...
	 * line does not intersect the neares line segment inside its segment bounds or
	 * if p_from_point is contained in this polyline.
	 */
	...
	/**
	 * Shortens this polyline to p_new_line_count lines. Additioanally the last line
	...
	public String toString() {
		...
		for (FloatPoint line : precalculated_float_corners) {
			System.out.println(line);
		}
		for (Point line : precalculated_corners) {
			System.out.println(line);
		}
		System.out.println(precalculated_bounding_box);
		System.out.println(USE_BOUNDING_OCTAGON_FOR_OFFSET_SHAPES);
