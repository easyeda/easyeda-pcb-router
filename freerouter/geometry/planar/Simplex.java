...
public class Simplex extends TileShape implements java.io.Serializable {
	 ...
	 * if possible. (For example a Simplex to an IntOctagon).
	 */
	@Override
	...
	/**
	 * Returns true, if the determinant of the direction of index p_no -1 and the
	 * direction of index p_no is > 0
	 */
	@Override
	 ...
	 * simplex. If the simplex is not bounded at this corner, the coordinates of the
	 ...
	 * p_no-th line of this simplex by a FloatPoint. If the simplex is not bounded
	...
	/**
	 * returns the p_no-th edge line of this simplex. The edge lines are sorted in
	 * ascending direction.
	 */
	@Override
	...
	/**
	 * Returns the dimension of this simplex. The result may be 2, 1, 0, or -1 (if
	...
	/**
	 * Returns the simplex, which results from translating the lines of this simplex
	 * by p_vector
	 */
	@Override
	...
	/**
	 * Returns the smallest box with int coordinates containing all corners of this
	 * simplex. The coordinates of the result will be Integer.MAX_VALUE, if the
	...
	/**
	 * Calculates a bounding octagon of the Simplex. Returns null, if the Simplex is
	 * not bounded.
	 */
	@Override
	public IntOctagon bounding_octagon() {
		if (precalculated_bounding_octagon == null) {
			...
			if (Math.min(lx, ly) < -Limits.CRIT_INT || Math.max(rx, uy) > Limits.CRIT_INT
					|| Math.min(ulx, llx) < -Limits.CRIT_INT || Math.max(lrx, urx) > Limits.CRIT_INT)
			// result is not bounded
			{
				return null;
			}
			precalculated_bounding_octagon = new IntOctagon((int) Math.floor(lx), (int) Math.floor(ly),
					(int) Math.ceil(rx), (int) Math.ceil(uy), (int) Math.floor(ulx), (int) Math.ceil(lrx),
	...
	/**
	 * Returns the simplex offseted by p_with. If p_width > 0, the offset is to the
	 * outer, else to the inner.
	 */
	@Override
	...
	/**
	 * Returns this simplex enlarged by p_offset. The result simplex is intersected
	 * with the by p_offset enlarged bounding octagon of this simplex
	 */
	@Override
	...
	/**
	 * Returns the number of the rightmost corner seen from p_from_point No other
	 * point of this simplex may be to the right of the line from p_from_point to
	 * the result corner.
	 */
	...
	/**
	 * Enlarges the simplex by removing the edge line with index p_no. The result
	 * simplex may get unbounded.
	 */
	...
	/**
	 * Constructs a Simplex from the directed lines in p_line_arr. The simplex will
	 * not be normalized. To get a normalised simplex use TileShape.get_instance
	 */
	...
	/**
	 * cuts this simplex out of p_outer_simplex. Divides the resulting shape into
	 * simplices along the minimal distance lines from the vertices of the inner
	 * simplex to the outer simplex; Returns the convex pieces constructed by this
	 * division.
	 */
	@Override
	public Simplex[] cutout_from(Simplex p_outer_simplex) {
		...
		for (int inner_corner_no = 0; inner_corner_no < inner_corner_count; ++inner_corner_no) {
			...
			// inner_simplex.line [inner_corner_no] and the last current
			// division line
			// and intersect it with the outer simplex
			...
			if (!check_cross_first_line) {
				check_cross_first_line = inner_corner_no > 0 && last_curr_dir.determinant(first_direction) > 0
	...
	Simplex remove_redundant_lines() {
		...
		while (try_again) {
			...
			for (int ind = 0; ind < new_length; ++ind) {
				...
				if (det != 0) // prev_line and next_line are not parallel
				{
					...
					if (det > 0)
					// direction of next_line is bigger than direction of
					// prev_line
					{
					...
					// direction of next_line is smaller than direction of
					// prev_line
					{
						...
						if (intersection_sides[ind] == Side.ON_THE_LEFT) {
							...
							// direction of curr_line is bigger than direction
							// of prev_line
							{
								// the halfplane defined by curr_line does not
								// intersect
								// with the simplex defined by prev_line and
								// nex_line,
		...
		if (new_length == 2) {
			if (line_arr[0].is_parallel(line_arr[1])) {
				...
				{
					if (line_arr[1].side_of(line_arr[0].a) == Side.COLLINEAR) {
	...
	/**
	 * For each corner of this inner simplex 1 or 2 perpendicular projections onto
	 * lines of the outer simplex are constructed, so that the resulting pieces
	 * after cutting out the inner simplex are convex. 2 projections may be
	 * nessesary at sharp angle corners. Used in in the method cutout_from with
	 * parametertype Simplex.
	 */
	private Line[] calc_division_lines(int p_inner_corner_no, Simplex p_outer_simplex) {
		...
		boolean is_exact = Math.abs(inner_corner.x - intersection.x) < c_tolerance
		...
		for (int ind = 0; ind < p_outer_simplex.arr.length; ++ind) {
			...
			if (projection_visible) {
				...
				if (second_division_necessary) {
					// search the first projection_dir between
					// curr_projection_dir
					...
					while (!second_projection_visible) {
						...
						curr_second_projection_dir = (IntDirection) inner_corner
								.perpendicular_direction(p_outer_simplex.arr[tmp_outer_line_no]);
						...
						if (curr_projection_dir.determinant(curr_second_projection_dir) <= 0) {
							...
							// curr_second_projection_dir would be already
							// bigger
							// than 180 degree
					...
					curr_distance += Math
							.abs(p_outer_simplex.arr[tmp_outer_line_no].signed_distance(inner_corner.to_float()));
				}
				if (curr_distance < min_distance) {
					min_distance = curr_distance;
