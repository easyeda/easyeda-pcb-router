...
public class LineSegment implements java.io.Serializable {
	...
	/**
	 * Creates a line segment from the 3 input lines. It starts at the intersection
	 * of p_start_line and p_middle_line and ends at the intersection of
	 * p_middle_line and p_end_line. p_start_line and p_end_line must not be
	 * parallel to p_middle_line.
	 */
	...
	public IntOctagon bounding_octagon() {
		...
		IntOctagon result = new IntOctagon((int) lx, (int) ly, (int) rx, (int) uy, (int) ulx, (int) lrx, (int) llx,
				(int) urx);
	 ...
	 * array may have length 0, 1 or 2. If the segments do not intersect the result
	 * array will have length 0. The result lines are so that the intersections of
	 * the result lines with this line segment will deliver the intersection points.
	 * If the segments overlap, the result array has length 2 and the intersection
	 * points are the first and the last overlap point. Otherwise the result array
	 * has length 1 and the intersection point is the the unique intersection or
	 * touching point. The result is not symmetric in this and p_other, because
	 * intersecting lines and not the intersection points are returned.
	 */
	public Line[] intersection(LineSegment p_other) {
		...
		if (start_point_side == Side.COLLINEAR && end_point_side == Side.COLLINEAR) {
			...
			if (cmp < 0) {
				// end point of the left line is to the lsft of the start point
				// of the right line
				return new Line[0];
			}
			if (cmp == 0) {
				// end point of the left line is equal to the start point of the
				// right line
		...
		// now both start points and both end points are on different sides of
		// the middle
	...
	/**
	 * Checks if this LineSegment and p_other contain a common LineSegment, which is
	...
	/**
	 * Constructs an approximation of this line segment by orthogonal stairs with
	 * integer coordinates. The length of the stairs will be at most p_stair_width.
	 * If p_to_the_right, the stairs will be to the right of this line segment, else
	 * to the left.
	 */
	 ...
	 * intersected by this line segment. Intersections at an endpoint of this line
	 * segment are only counted, if the line segment intersects with the interiour
	 * of p_shape. The result array may have lenght 0, 1 or 2. With 2 intersections
	 * the intersection which is nearest to the start point of the line segment
	 * comes first.
	 */
			...
			if (start_point_side != Side.ON_THE_RIGHT || end_point_side != Side.ON_THE_RIGHT) {
				...
				if (prev_line_side_of_is != Side.ON_THE_LEFT && next_line_side_of_is != Side.ON_THE_LEFT) {
					...
					if (prev_line_side_of_is == Side.COLLINEAR) {
						...
						if (prev_prev_corner_side == Side.COLLINEAR || next_corner_side == Side.COLLINEAR
					...
					if (next_line_side_of_is == Side.COLLINEAR) {
						...
						if (prev_corner_side == Side.COLLINEAR || next_next_corner_side == Side.COLLINEAR
	...
	public String toString() {
		...
		return "start: " + start + "," + "middle: " + middle + "," + "end: " + end + "\n" + "precalculated_start_point"
				+ precalculated_start_point + ", precalculated_end_point" + precalculated_end_point;
	}
