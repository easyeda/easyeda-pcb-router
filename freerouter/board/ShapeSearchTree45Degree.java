...
public class ShapeSearchTree45Degree extends ShapeSearchTree {
		...
		for (;;) {
			...
			if (curr_node.bounding_shape.intersects(bounding_shape)) {
				if (curr_node instanceof Leaf) {
					...
					if (is_obstacle && curr_object.shape_layer(shape_index) == room_layer
							...
							if (curr_shape.overlaps(curr_object_shape)) {
								if (curr_object instanceof CompleteFreeSpaceExpansionRoom && p_ignore_shape != null) {
									...
									if (p_ignore_shape.contains(intersection)) {
										// ignore also all objects, whose
										// intersection is contained in the
										// 2-dim overlap-door with the
										// from_room.
								...
								Collection<IncompleteFreeSpaceExpansionRoom> new_restrained_shapes = restrain_shape(
										curr_room, curr_object_shape);
	...
	protected Collection<IncompleteFreeSpaceExpansionRoom> divide_large_room(
		...
		Collection<IncompleteFreeSpaceExpansionRoom> result = super.divide_large_room(p_room_list,
				p_board_bounding_box);
		for (IncompleteFreeSpaceExpansionRoom curr_room : result) {
	 ...
	 */
	private static boolean obstacle_segment_touches_inside(IntOctagon p_obstacle_shape, int p_obstacle_border_line_no,
	...
	private Collection<IncompleteFreeSpaceExpansionRoom> restrain_shape(
		...
		// Search the edge line of p_obstacle_shape, so that
		// p_shape_to_be_contained
		...
		if (cut_line_distance >= 0) {
			IntOctagon restrained_shape = calc_outside_restrained_shape(p_obstacle_shape, restraining_line_no,
					room_shape);
			result.add(new IncompleteFreeSpaceExpansionRoom(restrained_shape, p_incomplete_room.get_layer(),
		...
		if (shape_to_be_contained.dimension() < 1) {
			// There is already a completed expansion room around
			// p_shape_to_be_contained.
			return result;
		}
		...
		for (int obstacle_line_no = 0; obstacle_line_no < 8; ++obstacle_line_no) {
			if (obstacle_segment_touches_inside(p_obstacle_shape, obstacle_line_no, room_shape)) {
				...
				if (shape_to_be_contained.side_of(curr_line) == Side.COLLINEAR) {
					// curr_line intersects with the interiour of
					// p_shape_to_be_contained
					restraining_line_no = obstacle_line_no;
					break;
				}
		...
		if (rest_piece.dimension() >= 2) {
			...
			if (rest_shape_to_be_contained.dimension() >= 0) {
				IncompleteFreeSpaceExpansionRoom rest_incomplete_room = new IncompleteFreeSpaceExpansionRoom(rest_piece,
						p_incomplete_room.get_layer(), rest_shape_to_be_contained);
	...
	private static double signed_line_distance(IntOctagon p_obstacle_shape, int p_obstacle_line_no,
			IntOctagon p_contained_shape) {
		double result;
		...
		// factor 0.5 used instead to 1 / sqrt(2) to prefer orthogonal lines
		// slightly to diagonal restraining lines.
	 ...
	 */
	IntOctagon calc_outside_restrained_shape(IntOctagon p_obstacle_shape, int p_obstacle_line_no,
			IntOctagon p_room_shape) {
		...
		if (p_obstacle_line_no == 0) {
			...
			System.out
					.println("ShapeSearchTree45Degree.calc_outside_restrained_shape: p_obstacle_line_no out of range");
		}
	 ...
	 * borderline with index p_obstacle_line_no of p_obstacle_shape.
	 */
	IntOctagon calc_inside_restrained_shape(IntOctagon p_obstacle_shape, int p_obstacle_line_no,
			IntOctagon p_room_shape) {
		int lx = p_room_shape.lx;
		int ly = p_room_shape.ly;
		int rx = p_room_shape.rx;
		...
		for (int i = 0; i < result.length; ++i) {
			...
			if (curr_shape == null) {
				...
				if (curr_tile_shape.is_IntBox()) {
					...
					// To avoid small corner cutoffs when taking the offset as
					// an octagon.
					// That may complicate the room division in the maze expand
					// algorithm unnecessesary.
				}
