...
public class MoveComponent {
	...
	public MoveComponent(Item p_item, Vector p_translate_vector, int p_max_recursion_depth,
		...
		for (Item curr_item : item_group_list) {
			boolean curr_item_movable = !curr_item.is_user_fixed() && ((curr_item instanceof DrillItem)
					|| (curr_item instanceof ObstacleArea) || (curr_item instanceof ComponentOutline));
		...
		for (int i = 0; i < item_group_arr.length; ++i) {
			...
			if (item_group_arr[i].item instanceof DrillItem) {
				...
				if (this.translate_vector.length_approx() >= curr_drill_item.min_width()) {
					...
					move_ok = MoveDrillItemAlgo.check(curr_drill_item, this.translate_vector, this.max_recursion_depth,
							this.max_via_recursion_depth, ignore_items, board, time_limit);
				}
			} else {
	...
	public boolean insert(int p_tidy_width, int p_pull_tight_accuracy) {
		...
		if (this.component != null) {
			// component must be moved first, so that the new pin shapes are
			// calculeted correctly
		...
		for (int i = 0; i < item_group_arr.length; ++i) {
			if (item_group_arr[i].item instanceof DrillItem) {
				DrillItem curr_drill_item = (DrillItem) item_group_arr[i].item;
				boolean move_ok = board.move_drill_item(curr_drill_item, this.translate_vector,
						this.max_recursion_depth, this.max_via_recursion_depth, p_tidy_width, p_pull_tight_accuracy,
						PULL_TIGHT_TIME_LIMIT);
				if (!move_ok) {
					if (this.component != null) {
						this.component.translate_by(translate_vector.negate());
						// Otherwise the component outline is not restored
						// correctly by the undo algorithm.
					}
					return false;
				}
			} else {
