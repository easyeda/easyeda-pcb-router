...
public class ForcedPadAlgo {
	 ...
	 */
	public CheckDrillResult check_forced_pad(TileShape p_pad_shape, CalcFromSide p_from_side, int p_layer,
			int[] p_net_no_arr, int p_cl_type, boolean p_copper_sharing_allowed, Collection<Item> p_ignore_items,
			int p_max_recursion_depth, int p_max_via_recursion_depth, boolean p_check_only_front,
		...
		ShapeTraceEntries shape_entries = new ShapeTraceEntries(p_pad_shape, p_layer, p_net_no_arr, p_cl_type,
				p_from_side, board);
		Collection<Item> obstacles = search_tree.overlapping_items_with_clearance(p_pad_shape, p_layer, new int[0],
				p_cl_type);
		...
		for (Via curr_shove_via : shape_entries.shove_via_list) {
			...
			IntPoint[] new_via_center = MoveDrillItemAlgo.try_shove_via_points(p_pad_shape, p_layer, curr_shove_via,
					p_cl_type, false, board);
			...
			if (!MoveDrillItemAlgo.check(curr_shove_via, delta, p_max_recursion_depth, p_max_via_recursion_depth - 1,
					ignore_items, this.board, p_time_limit)) {
				return CheckDrillResult.NOT_DRILLABLE;
			}
		}
		...
		for (;;) {
			...
			for (int i = 0; i < curr_substitute_trace.tile_shape_count(); ++i) {
				...
				if (p_check_only_front) {
					is_in_front = in_front_of_pad(curr_line, p_pad_shape, p_from_side.no,
							curr_substitute_trace.get_half_width(), true);
				} else {
					is_in_front = true;
				}
				if (is_in_front) {
	 ...
	 * necessesary.
	 */
	boolean forced_pad(TileShape p_pad_shape, CalcFromSide p_from_side, int p_layer, int[] p_net_no_arr, int p_cl_type,
			boolean p_copper_sharing_allowed, Collection<Item> p_ignore_items, int p_max_recursion_depth,
		...
		if (!MoveDrillItemAlgo.shove_vias(p_pad_shape, p_from_side, p_layer, p_net_no_arr, p_cl_type, p_ignore_items,
				p_max_recursion_depth, p_max_via_recursion_depth, false, this.board)) {
			return false;
		}
		ShapeSearchTree search_tree = this.board.search_tree_manager.get_default_tree();
		ShapeTraceEntries shape_entries = new ShapeTraceEntries(p_pad_shape, p_layer, p_net_no_arr, p_cl_type,
		...
		for (;;) {
			...
			for (int i = 0; i < curr_substitute_trace.tile_shape_count(); ++i) {
				CalcShapeAndFromSide curr = new CalcShapeAndFromSide(curr_substitute_trace, i, is_orthogonal_mode,
						false);
				if (!shove_trace_algo.insert(curr.shape, curr.from_side, p_layer, curr_net_no_arr,
	...
	CalcFromSide calc_from_side(TileShape p_shape, Point p_shape_center, int p_layer, int p_offset, int p_cl_class) {
		...
		for (int i = 0; i < offset_shape.border_line_count(); ++i) {
			TileShape check_shape = calc_check_chape_for_from_side(p_shape, p_shape_center,
					offset_shape.border_line(i));
	...
	private static TileShape calc_check_chape_for_from_side(TileShape p_shape, Point p_shape_center,
			Line p_border_line) {
	 ...
	 * p_from_side
	 */
	private static boolean in_front_of_pad(Line p_line, TileShape p_pad_shape, int p_from_side, int p_width,
			boolean p_with_sides) {
		...
		case 0:
			result = Math.min(line_a.y, line_b.y) >= pad_octagon.uy + p_width
					|| Math.max(line_a.x - line_a.y, line_b.x - line_b.y) <= pad_octagon.ulx - diag_width
					|| Math.min(line_a.x + line_a.y, line_b.x + line_b.x) >= pad_octagon.urx + diag_width;
			if (p_with_sides && !result) {
				result = Math.max(line_a.x, line_b.x) <= pad_octagon.lx - p_width
						&& Math.min(line_a.x - line_a.y, line_b.x - line_b.y) <= pad_octagon.ulx - diag_width
						|| Math.min(line_a.x, line_b.x) >= pad_octagon.rx + p_width
								&& Math.min(line_a.x + line_a.y, line_b.x + line_b.y) >= pad_octagon.urx + diag_width;
			}
			break;
		case 1:
			result = Math.min(line_a.y, line_b.y) >= pad_octagon.uy + p_width
			...
			if (p_with_sides && !result) {
				result = Math.min(line_a.x, line_b.x) <= pad_octagon.lx - p_width
						&& Math.max(line_a.x + line_a.y, line_b.x + line_b.y) <= pad_octagon.llx - diag_width
		...
		case 2:
			...
			if (p_with_sides && !result) {
				result = Math.max(line_a.y, line_b.y) <= pad_octagon.ly - p_width
						&& Math.min(line_a.x + line_a.y, line_b.x + line_b.y) <= pad_octagon.llx - diag_width
		...
		case 3:
			...
			if (p_with_sides && !result) {
				result = Math.min(line_a.y, line_b.y) <= pad_octagon.ly - p_width
						&& Math.min(line_a.x - line_a.y, line_b.x - line_b.y) >= pad_octagon.lrx + diag_width
		...
		case 4:
			...
			if (p_with_sides && !result) {
				result = Math.min(line_a.x, line_b.x) >= pad_octagon.rx + p_width
						&& Math.max(line_a.x - line_a.y, line_b.x - line_b.y) >= pad_octagon.lrx + diag_width
						|| Math.max(line_a.x, line_b.x) <= pad_octagon.lx - p_width
								&& Math.min(line_a.x + line_a.y, line_b.x + line_b.y) <= pad_octagon.llx - diag_width;
			}
			break;
		...
		case 6:
			...
			if (p_with_sides && !result) {
				result = Math.max(line_a.y, line_b.y) <= pad_octagon.ly - p_width
						&& Math.max(line_a.x - line_a.y, line_b.x - line_b.y) >= pad_octagon.lrx + diag_width
						|| Math.min(line_a.y, line_b.y) >= pad_octagon.uy + p_width
								&& Math.max(line_a.x + line_a.y, line_b.x + line_b.y) >= pad_octagon.urx + diag_width;
			}
			break;
		case 7:
			result = Math.min(line_a.y, line_b.y) >= pad_octagon.uy + p_width
