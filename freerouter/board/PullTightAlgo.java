...
public abstract class PullTightAlgo {
	 ...
	 */
	static PullTightAlgo get_instance(RoutingBoard p_board, int[] p_only_net_no_arr, IntOctagon p_clip_shape,
			int p_min_translate_dist, Stoppable p_stoppable_thread, int p_time_limit, Point p_keep_point,
		...
		if (angle_restriction == AngleRestriction.NINETY_DEGREE) {
			result = new PullTightAlgo90(p_board, p_only_net_no_arr, p_stoppable_thread, p_time_limit, p_keep_point,
					p_keep_point_layer);
		} else if (angle_restriction == AngleRestriction.FORTYFIVE_DEGREE) {
			result = new PullTightAlgo45(p_board, p_only_net_no_arr, p_stoppable_thread, p_time_limit, p_keep_point,
					p_keep_point_layer);
		} else {
	...
	void opt_changed_area(ExpansionCostFactor[] p_trace_cost_arr) {
		...
		while (something_changed) {
			...
			for (int i = 0; i < board.get_layer_count(); ++i) {
				...
				double changed_area_offset = 1.5
						* (board.rules.clearance_matrix.max_value(i) + 2 * board.rules.get_max_trace_half_width());
				...
				while (it.hasNext()) {
					...
					if (curr_ob instanceof PolylineTrace) {
						...
						if (OptViaAlgo.opt_via_location(this.board, (Via) curr_ob, p_trace_cost_arr,
								this.min_translate_dist, 10)) {
							something_changed = true;
						}
					}
				}
	 ...
	 */
	Polyline pull_tight(Polyline p_polyline, int p_layer, int p_half_width, int[] p_net_no_arr, int p_cl_type,
	...
	protected boolean is_stop_requested() {
		...
		if (time_limit_exceeded
				&& this.board.get_test_level().ordinal() >= TestLevel.CRITICAL_DEBUGGING_OUTPUT.ordinal()) {
	...
	protected Line reposition_line(Line[] p_line_arr, int p_no) {
		...
		while (first_time || Math.abs(delta_dist) > min_translate_dist) {
			...
			if (tmp.arr.length == 3) {
				...
				check_ok = board.check_trace_shape(shape_to_check, curr_layer, curr_net_no_arr, curr_cl_type,
						this.contact_pins);
		...
		for (int i = 1; i < curr_polyline.arr.length - 1; ++i) {
			...
			if (try_skip) {
				...
				if (check_ok && !curr_polyline.arr[i].is_multiple_of_45_degree()) {
					// no check necessary for skipping 45 degree lines, because
					// the check is
	...
	private boolean smoothen_end_corners_at_trace_1(PolylineTrace p_trace) {
		...
		// to allow the trace to slide to the end point of a contact trace, if
		// the contact trace ends at a pin.
		...
		while (connection_to_trace_improved) {
			...
			if (adjusted_polyline != null) {
				...
				curr_trace = board.insert_trace_without_cleaning(adjusted_polyline, trace_layer, curr_half_width,
						curr_trace.net_no_arr, curr_cl_class, curr_fixed_state);
	...
	protected Polyline avoid_acid_traps(Polyline p_polyline) {
		...
		Polyline new_polyline = shove_trace_algo.spring_over_obstacles(p_polyline, curr_half_width, curr_layer,
				curr_net_no_arr, curr_cl_type, contact_pins);
		if (new_polyline != null && new_polyline != p_polyline) {
			if (this.board.check_polyline_trace(new_polyline, curr_layer, curr_half_width, curr_net_no_arr,
					curr_cl_type)) {
				result = new_polyline;
			}
		}
