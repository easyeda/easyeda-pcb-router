...
public class MoveDrillItemAlgo {
	...
	public static boolean check(DrillItem p_drill_item, Vector p_vector, int p_max_recursion_depth,
			int p_max_via_recursion_depth, Collection<Item> p_ignore_items, RoutingBoard p_board,
		...
		for (int curr_layer = p_drill_item.first_layer(); curr_layer <= p_drill_item.last_layer(); ++curr_layer) {
			...
			if (forced_pad_algo.check_forced_pad(curr_tile_shape, from_side, curr_layer, p_drill_item.net_no_arr,
					p_drill_item.clearance_class_no(), attach_allowed, ignore_items, p_max_recursion_depth,
					p_max_via_recursion_depth, true, p_time_limit) == ForcedPadAlgo.CheckDrillResult.NOT_DRILLABLE) {
				return false;
			}
		}
		return true;
	 ...
	 */
	static boolean insert(DrillItem p_drill_item, Vector p_vector, int p_max_recursion_depth,
			int p_max_via_recursion_depth, IntOctagon p_tidy_region, RoutingBoard p_board) {
		if (p_drill_item.is_shove_fixed()) {
			return false;
		}
	...
	static boolean shove_vias(TileShape p_obstacle_shape, CalcFromSide p_from_side, int p_layer, int[] p_net_no_arr,
			...
			boolean p_copper_sharing_allowed, RoutingBoard p_board) {
		ShapeSearchTree search_tree = p_board.search_tree_manager.get_default_tree();
		ShapeTraceEntries shape_entries = new ShapeTraceEntries(p_obstacle_shape, p_layer, p_net_no_arr, p_cl_type,
				p_from_side, p_board);
		Collection<Item> obstacles = search_tree.overlapping_items_with_clearance(p_obstacle_shape, p_layer, new int[0],
				p_cl_type);
		...
		for (Via curr_via : shape_entries.shove_via_list) {
			...
			IntPoint[] try_via_centers = try_shove_via_points(p_obstacle_shape, p_layer, curr_via, p_cl_type, true,
					p_board);
			...
			for (int i = 0; i < try_via_centers.length; ++i) {
				if (i == 0 || check_via_center.distance_square(try_via_centers[i].to_float()) <= max_dist_square) {
					...
					// No time limit here because the item database is already
					// changed.
					boolean shove_ok = check(curr_via, rel_coor, p_max_recursion_depth, p_max_via_recursion_depth - 1,
							ignore_items, p_board, null);
					if (shove_ok) {
	...
	static IntPoint[] try_shove_via_points(TileShape p_obstacle_shape, int p_layer, Via p_via, int p_cl_class_no,
		...
		if (p_board.rules.get_trace_angle_restriction() == AngleRestriction.NINETY_DEGREE || is_int_octagon) {
			...
			shove_distance = 0;
			if (!search_tree.is_clearance_compensation_used()) {
				// enlarge p_obstacle_shape and curr_via_shape by half of the
				// clearance value to syncronize
				// with the check algorithm in
				// ShapeSearchTree.overlapping_tree_entries_with_clearance
				shove_distance += 0.5 * clearance_value;
			}
		}

		// The additional constant 2 is an empirical value for the tolerance in
		// case of diagonal shoving.
		shove_distance += 2;
