...
public class ShapeSearchTree extends datastructures.MinAreaTree {
	...
	public int clearance_compensation_value(int p_clearance_class_no, int p_layer) {
		...
		int result = board.rules.clearance_matrix.value(p_clearance_class_no, this.compensated_clearance_class_no,
				p_layer)
				- board.rules.clearance_matrix.clearance_compensation_value(this.compensated_clearance_class_no,
						p_layer);
	...
	void merge_entries_in_front(PolylineTrace p_from_trace, PolylineTrace p_to_trace, Polyline p_joined_polyline,
		...
		TileShape[] new_precalculated_tree_shapes = new TileShape[new_shape_count];
		// transfer the tree entries except the last or first from p_from_trace
		// to p_to_trace
	 ...
	 */
	void reuse_entries_after_cutout(PolylineTrace p_from_trace, PolylineTrace p_start_piece,
			PolylineTrace p_end_piece) {
					...
					if (curr_object.is_trace_obstacle(p_net_no) && curr_object.shape_layer(shape_index) == room_layer
						...
						for (IncompleteFreeSpaceExpansionRoom curr_incomplete_room : result) {
							...
							if (intersection.dimension() == 2) {
								...
								// cannot happen in free angle roouting, because
								// then expansion_rooms
								// may not overlap. Therefore that can be
								// removed as soon as special
								...
								if (!ignore_expansion_room) {
									...
									for (IncompleteFreeSpaceExpansionRoom tmp_room : new_result) {
										new_bounding_shape = new_bounding_shape
												.union(tmp_room.get_shape().bounding_shape(this.bounding_directions));
									}
								}
							}
	...
	private Collection<IncompleteFreeSpaceExpansionRoom> restrain_shape(
		...
		Simplex obstacle_simplex = p_obstacle_shape.to_Simplex(); // otherwise
																	// border_lines
																	// of lenth
																	// 0 for
																	// octagons
																	...
																	// handeled
																	// correctly
		TileShape shape_to_be_contained = p_incomplete_room.get_contained_shape().to_Simplex(); // There
																								// may
																								// be
																								// a
																								// performance
		...
		if (cut_line != null) {
			...
			// completely on the right side of that line. Search a cut line, so
			// that
			...
			if (cut_line == null) {
				// cut line not found, parts or the whole of p_shape may be
				// already
	...
	TileShape[] calculate_tree_shapes(ObstacleArea p_obstacle_area) {
		...
		if (this.board.communication.host_cad_exists()) {
			max_tree_shape_width = Math.min(500 * this.board.communication.get_resolution(Unit.MIL),
					max_tree_shape_width);
			// Problem with low resolution on Kicad.
			// Called only for designs from host cad systems because otherwise
			// the old sample.dsn gets to many tree shapes.
		}
	...
	TileShape[] calculate_tree_shapes(BoardOutline p_board_outline) {
		...
		if (p_board_outline.keepout_outside_outline_generated()) {
			...
			// Only the line shapes of the outline are inserted as obstales into
			// the tree.
