...
class PullTightAlgo45 extends PullTightAlgo {
	...
	public PullTightAlgo45(RoutingBoard p_board, int[] p_only_net_no_arr, Stoppable p_stoppable_thread,
			int p_time_limit, Point p_keep_point, int p_keep_point_layer) {
	...
	private Polyline reduce_corners(Polyline p_polyline) {
		...
		while (corner_no < p_polyline.arr.length - 1) {
			...
			if (curr_corner[1].equals(curr_corner[2]) || corner_no < p_polyline.arr.length - 2
			...
			if (curr_corner_in_clip_shape[1] && curr_corner_in_clip_shape[2] && curr_corner_in_clip_shape[3]) {
				// translate the line from curr_corner[2] to curr_corner[1] to
				// curr_corner[3]
						...
						if (board.check_trace_shape(shape_to_check, curr_layer, curr_net_no_arr, curr_cl_type,
							...
							if (curr_check_points[0].equals(curr_check_points[1])) {
								...
								if (check_polyline.arr.length == 3) {
									...
									corner_removed = board.check_trace_shape(shape_to_check, curr_layer,
											curr_net_no_arr, curr_cl_type, this.contact_pins);
								} else {
									corner_removed = true;
								}
							}
			...
			if (!corner_removed && curr_corner_in_clip_shape[0] && curr_corner_in_clip_shape[1]
					&& curr_corner_in_clip_shape[2]) {
	...
	private Polyline smoothen_corners(Polyline p_polyline) {
		...
		while (polyline_changed) {
			...
			for (int i = 1; i < line_arr.length - 2; ++i) {
				...
				if (d1.is_multiple_of_45_degree() && d2.is_multiple_of_45_degree()
	...
	private Line smoothen_corner(Line[] p_line_arr, int p_no) {
		...
		if (result != null && board.changed_area != null) {
			FloatPoint new_prev_corner = check_lines[0].intersection_approx(result);
			FloatPoint new_next_corner = check_lines[2].intersection_approx(result);
