...
public class Component implements UndoableObjects.Storable, ObjectInfoPanel.Printable, java.io.Serializable {
	 ...
	 */
	Component(String p_name, Point p_location, double p_rotation_in_degree, boolean p_on_front, Package p_package_front,
			Package p_package_back, int p_no, boolean p_position_fixed) {
		name = p_name;
		location = p_location;
	...
	public void rotate(double p_angle_in_degree, IntPoint p_pole, boolean p_flip_style_rotate_first) {
		...
		if (p_flip_style_rotate_first && !this.placed_on_front()) {
			// take care of the order of mirroring and rotating on the back side
			// of the board
		...
		if (location != null) {
			this.location = this.location.to_float().rotate(Math.toRadians(p_angle_in_degree), p_pole.to_float())
					.round();
		}
	}
	...
	public Component clone() {
		Component result = new Component(name, location, rotation_in_degree, on_front, lib_package_front,
				lib_package_back, no, position_fixed);
