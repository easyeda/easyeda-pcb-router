...
public class ShapeTraceEntries {
	...
	PolylineTrace next_substitute_trace_piece() {
		...
		return new PolylineTrace(piece_polyline, this.layer, curr_trace.get_half_width(), curr_trace.net_no_arr,
		...
		if (pieces.length == 2 && offset_shape.is_outside(pieces[0].first_corner())
			...
			for (int i = 0; i < pieces.length; ++i) {
				board.insert_trace_without_cleaning(pieces[i], p_trace.get_layer(), p_trace.get_half_width(),
						p_trace.net_no_arr, p_trace.clearance_class_no(), FixedState.UNFIXED);
			}
		}
	}
				...
				if (offset_shape.contains(end_corner)) {
					...
					while (it.hasNext()) {
						...
						if (contact_item instanceof Trace) {
							...
							double via_trace_diff = via_shape.smallest_radius()
									- p_trace.get_compensated_half_width(search_tree);
							if (!search_tree.is_clearance_compensation_used()) {
								int via_clearance = board.clearance_value(contact_item.clearance_class_no(),
										this.cl_class, this.layer);
								int trace_clearance = board.clearance_value(p_trace.clearance_class_no(), this.cl_class,
										this.layer);
					...
					if (contact_count == 1 && store_end_corner) {
						Point projection = offset_shape.nearest_border_point(end_corner);
						{
							...
							// the following may not be correct because the
							// trace may not conntain a suitable
							// line for the construction oof the end line of the
							// substitute trace.
							if (i == 0) {
							...
							if (projection_side >= 0) {
								insert_entry_point(p_trace, trace_line_segment_no, projection_side,
										projection.to_float());
							}
						}
			...
			if (curr.edge_no == from_side.no) {
				if (from_side.border_intersection != null) {
					...
					if (curr_projection.distance_square(compare_corner_1) >= from_point_dist
							&& curr_projection.distance_square(from_point_projection) <= curr_projection
