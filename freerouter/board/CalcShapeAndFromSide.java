...
class CalcShapeAndFromSide {
	...
	CalcShapeAndFromSide(PolylineTrace p_trace, int p_index, boolean p_orthogonal, boolean p_in_shove_check) {
		...
		if (p_orthogonal) {
			...
			if (from_side_no >= 0) {
				FloatPoint border_intersection = curr_cut_line
						.intersection_approx(curr_shape.border_line(from_side_no));
				curr_from_side = new CalcFromSide(from_side_no, border_intersection);
			}
		}
		if (curr_from_side == null && !p_in_shove_check) {
			// In p_in_shove_check, using this calculation may produce an
			// undesired stack_level > 1 in ShapeTraceEntries.
	...
	private static Line calc_cutline_at_end(int p_index, PolylineTrace p_trace) {
		...
		ShapeSearchTree search_tree = p_trace.board.search_tree_manager.get_default_tree();
		if (p_index == trace_lines.arr.length - 3 || trace_lines.corner_approx(trace_lines.arr.length - 2)
				.distance(trace_lines.corner_approx(p_index + 1)) < p_trace.get_compensated_half_width(search_tree)) {
	...
	private static Line calc_cutline_at_start(int p_index, PolylineTrace p_trace) {
		Polyline trace_lines = p_trace.polyline();
		ShapeSearchTree search_tree = p_trace.board.search_tree_manager.get_default_tree();
		if (p_index == 0 || trace_lines.corner_approx(0).distance(trace_lines.corner_approx(p_index)) < p_trace
				.get_compensated_half_width(search_tree)) {
