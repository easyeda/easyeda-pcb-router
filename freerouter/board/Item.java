...
public abstract class Item
		implements Drawable, SearchTreeObject, ObjectInfoPanel.Printable, UndoableObjects.Storable, Serializable {
	...
	/**
	 * Returns the p_index-throws shape of this item after decomposition into convex
	 * polygonal shapes
	 */
	...
	/**
	 * Translates the shapes of this item by p_vector. Does not move the item in the
	 * board.
	 */
	public abstract void translate_by(Vector p_vector);

	/**
	 * Turns this Item by p_factor times 90 degree around p_pole. Does not update
	...
	/**
	 * Changes the placement side of this Item and mirrors it at the vertical line
	 * through p_pole. Does not update the item in the board.
	 */
	...
	/**
	 * Return the name of the component of this item or null, if this item does not
	...
	/**
	 * Returns a list of all clearance violations of this item with other items. The
	 * objects in the list are of type ClearanceViolations. The first_item in such
	 * an object is always this item.
	 */
	public Collection<ClearanceViolation> clearance_violations() {
		...
		for (int i = 0; i < tile_shape_count(); ++i) {
			...
			Collection<TreeEntry> curr_overlapping_items = default_tree.overlapping_tree_entries_with_clearance(
			...
			while (it.hasNext()) {
				...
				if (is_obstacle && this instanceof Trace && curr_item instanceof Trace) {
					...
					// In this case they are allowed to overlap without sharing
					// a net.
				...
				if (is_obstacle) {
					...
					if (!this.board.search_tree_manager.is_clearance_compensation_used()) {
						double cl_offset = 0.5 * board.rules.clearance_matrix.value(curr_item.clearance_class,
								this.clearance_class, shape_layer(i));
					...
					if (intersection.dimension() == 2) {
						ClearanceViolation curr_violation = new ClearanceViolation(this, curr_item, intersection,
								shape_layer(i));
	...
	/**
	 * Returns all connectable Items with a direct contacts to this item. The result
	 * will be empty, if this item is not connectable.
	 */
	public Set<Item> get_all_contacts() {
		...
		for (int i = 0; i < this.tile_shape_count(); ++i) {
			Collection<SearchTreeObject> overlapping_items = board.overlapping_objects(get_tile_shape(i),
					shape_layer(i));
	...
	/**
	 * Checks, if this item is electrically connected to another connectable item on
	 * the input layer. Returns false for items, which are not connectable.
	 */
	...
	/**
	 * Returns the contact point, if this item and p_other are Connectable and have
	 * a unique normal contact. Returns null otherwise
	 */
	 ...
	 * Returns the set of all Connectable items of the net with number p_net_no
	 * which can be reached recursively via normal contacts from this item. If
	 * p_net_no <= 0, the net number is ignored. If p_stop_at_plane, the recursive
	 * algorithm stops, when a conduction area is reached, which does not belong to
	 * a component.
	 */
	 ...
	 * p_net_no, which are not in the connected set of this item. If p_net_no <= 0,
	 * the net numbers contained in this items are used instead of p_net_no.
	 */
	...
	/**
	 * Returns all traces and vias from this item until the next fork or terminal
	 * item.
	 */
	...
	/**
	 * Returns all traces and vias from this item until the next fork or terminal
	 * item. If p_stop_option == StopConnectionOption.FANOUT_VIA, the algorithm will
	 * stop at the next fanout via, If p_stop_option == StopConnectionOption.VIA,
	...
	/**
	 * Returns for this item the layer of the shape with index p_index. If p_id_no
	 * <= 0, it w2ill be generated internally.
	 */
	@Override
	public abstract int shape_layer(int p_index);

	Item(int[] p_net_no_arr, int p_clearance_type, int p_id_no, int p_component_no, FixedState p_fixed_state,
			BasicBoard p_board) {
	...
	/**
	 * Returns true, if it is not allowed to change this item except evtl. shoving
	 * the item
	 */
	 ...
	 */
	public void unfix() {
		if (fixed_state != FixedState.SYSTEM_FIXED || fixed_state != FixedState.USER_FIXED) {
	...
	/**
	 * Removes p_net_no from the net number array. Returns false, if p_net_no was
	 * not contained in this array.
	 */
	...
	/**
	 * Returns the index in the clearance matrix describing the required spacing of
	...
	public void change_clearance_class(int p_index) {
		...
		if (this.board != null && this.board.search_tree_manager.is_clearance_compensation_used()) {
			// reinsert the item into the search tree, because the compensated
			// shape has changed.
	...
	/**
	 * Makes this item connectable and assigns it to the input net. If p_net_no < 0,
	 * the net items net number will be removed and the item will no longer be
	 * connectable.
	 */
	...
	/**
	 * Gets the information for the autoroute algorithm. Creates it, if it does not
	 * yet exist.
	 */
	...
	/**
	 * Clear all cached or derived data. so that they have to be recalculated, when
	...
	protected void print_net_info(ObjectInfoPanel p_window, java.util.Locale p_locale) {
		java.util.ResourceBundle resources = java.util.ResourceBundle.getBundle("board.resources.ObjectInfoPanel",
				p_locale);
	...
	protected void print_clearance_info(ObjectInfoPanel p_window, java.util.Locale p_locale) {
		if (this.clearance_class > 0) {
			...
			p_window.append(name, resources.getString("clearance_info"),
	...
	protected void print_fixed_info(ObjectInfoPanel p_window, java.util.Locale p_locale) {
		if (this.fixed_state != FixedState.UNFIXED) {
			java.util.ResourceBundle resources = java.util.ResourceBundle.getBundle("board.resources.FixedState",
					p_locale);
			p_window.append(", ");
	...
	boolean is_fanout_via(Set<Item> p_ignore_items) {
		...
		for (Item curr_contact : contact_list) {
			if (curr_contact instanceof Pin && curr_contact.first_layer() == curr_contact.last_layer()
			...
			if (curr_contact instanceof Trace) {
				...
				for (Item tmp_contact : trace_contact_list) {
					if (tmp_contact instanceof Pin && curr_contact.first_layer() == curr_contact.last_layer()
	...
	/**
	 * the index in the clearance matrix describing the required spacing to other
	 * items
	 */
	...
	/**
	 * Used as parameter of get_connection to control, that the connection stops at
	 * the next fanout via or at any via.
	 */
