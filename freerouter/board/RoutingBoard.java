...
public class RoutingBoard extends BasicBoard implements java.io.Serializable {
	...
	public RoutingBoard(IntBox p_bounding_box, LayerStructure p_layer_structure, PolylineShape[] p_outline_shapes,
		...
		super(p_bounding_box, p_layer_structure, p_outline_shapes, p_outline_cl_class_no, p_rules,
	...
	public void opt_changed_area(int[] p_only_net_no_arr, IntOctagon p_clip_shape, int p_accuracy,
		...
		opt_changed_area(p_only_net_no_arr, p_clip_shape, p_accuracy, p_trace_cost_arr, p_stoppable_thread,
				p_time_limit, null, 0);
	}
	 ...
	 */
	public void opt_changed_area(int[] p_only_net_no_arr, IntOctagon p_clip_shape, int p_accuracy,
			ExpansionCostFactor[] p_trace_cost_arr, Stoppable p_stoppable_thread, int p_time_limit, Point p_keep_point,
			int p_keep_point_layer) {
		if (changed_area == null) {
			return;
		}
		if (p_clip_shape != IntOctagon.EMPTY) {
			PullTightAlgo pull_tight_algo = PullTightAlgo.get_instance(this, p_only_net_no_arr, p_clip_shape,
	...
	public double check_trace_segment(LineSegment p_line_segment, int p_layer, int[] p_net_no_arr,
		...
		for (TreeEntry curr_obstacle_entry : obstacle_entries) {
			...
			if (default_tree.is_clearance_compensation_used()) {
				...
				shorten_value = p_trace_half_width + rules.clearance_matrix
						.clearance_compensation_value(curr_obstacle.clearance_class_no(), p_layer);
			} else {
	...
	public boolean check_move_item(Item p_item, Vector p_vector, Collection<Item> p_ignore_items) {
		...
		for (int i = 0; i < p_item.tile_shape_count(); ++i) {
			...
			Set<Item> obstacles = this.overlapping_items_with_clearance(moved_shape, p_item.shape_layer(i),
					p_item.net_no_arr, p_item.clearance_class_no());
	...
	public boolean check_change_net(Item p_item, int p_new_net_no) {
		...
		for (int i = 0; i < p_item.tile_shape_count(); ++i) {
			...
			for (SearchTreeObject curr_ob : obstacles) {
				if (curr_ob != p_item && curr_ob instanceof Connectable
	...
	public boolean move_drill_item(DrillItem p_drill_item, Vector p_vector, int p_max_recursion_depth,
		...
		start_marking_changed_area();
		if (!MoveDrillItemAlgo.insert(p_drill_item, p_vector, p_max_recursion_depth, p_max_via_recursion_depth,
				tidy_region, this)) {
			return false;
		}
		if (calculate_tidy_region) {
	...
	public Item pick_nearest_routing_item(Point p_location, int p_layer, Item p_from_item) {
		...
		while (it.hasNext()) {
			...
			if (candidate_found) {
				if (p_from_item != null) {
					if (ignore_set == null) {
						// calculated here to avoid unnessery calculations for
						// performance reasoss.
	...
	public boolean forced_via(ViaInfo p_via_info, Point p_location, int[] p_net_no_arr, int p_trace_clearance_class_no,
			int[] p_trace_pen_halfwidth_arr, int p_max_recursion_depth, int p_max_via_recursion_depth, int p_tidy_width,
			int p_pull_tight_accuracy, int p_pull_tight_time_limit) {
		clear_shove_failing_obstacle();
	...
	public Point insert_forced_trace_segment(Point p_from_corner, Point p_to_corner, int p_half_width, int p_layer,
			int[] p_net_no_arr, int p_clearance_class_no, int p_max_recursion_depth, int p_max_via_recursion_depth,
	...
	public boolean check_forced_trace_polyline(Polyline p_polyline, int p_half_width, int p_layer, int[] p_net_no_arr,
		...
		int compensated_half_width = p_half_width
				+ search_tree.clearance_compensation_value(p_clearance_class_no, p_layer);
	...
	public Point insert_forced_trace_polyline(Polyline p_polyline, int p_half_width, int p_layer, int[] p_net_no_arr,
		...
		// If so, its geometry will be used to cut off dog ears of the check
		// shape.
		...
		for (int i = 0; i < trace_shapes.length; ++i) {
			...
			if (p_with_check) {
				boolean check_shove_ok = shove_trace_algo.check(curr_trace_shape, from_side, null, p_layer,
						p_net_no_arr, p_clearance_class_no, p_max_recursion_depth, p_max_via_recursion_depth,
		...
		PullTightAlgo pull_tight_algo = PullTightAlgo.get_instance(this, opt_net_no_arr, tidy_region,
				p_pull_tight_accuracy, null, -1, new_corner, p_layer);

		// Remove evtl. generated cycles because otherwise pull_tight may not
		// work correctly.
		if (new_trace.normalize(changed_area.get_area(p_layer))) {

			pull_tight_algo.split_traces_at_keep_point();
			// otherwise the new corner may no more be contained in the new
			// trace after optimizing
		...
		// To avoid, that a separate handling for moving backwards in the own
		// trace line
	 ...
	 */
	public AutorouteEngine.AutorouteResult autoroute(Item p_item, interactive.Settings p_settings, int p_via_costs,
		...
		AutorouteControl ctrl_settings = new AutorouteControl(this, route_net_no, p_settings, p_via_costs,
		...
		if (route_net != null && route_net.contains_plane()) {
			for (Item curr_item : route_start_set) {
				if (curr_item instanceof board.ConductionArea) {
					return AutorouteEngine.AutorouteResult.ALREADY_CONNECTED; // already
																				// connected
																				// to
																				// plane
				}
		...
		if (route_dest_set.size() == 0) {
			return AutorouteEngine.AutorouteResult.ALREADY_CONNECTED; // p_item
																		// is
																		// already
																		// routed.
		}
		...
		AutorouteEngine.AutorouteResult result = curr_autoroute_engine.autoroute_connection(route_start_set,
				route_dest_set, ctrl_settings, ripped_item_list);
		if (result == AutorouteEngine.AutorouteResult.ROUTED) {
			final int time_limit_to_prevent_endless_loop = 1000;
			opt_changed_area(new int[0], null, p_settings.get_trace_pull_tight_accuracy(), ctrl_settings.trace_costs,
	...
	public AutorouteEngine.AutorouteResult fanout(Pin p_pin, interactive.Settings p_settings, int p_ripup_costs,
		...
		AutorouteEngine.AutorouteResult result = curr_autoroute_engine.autoroute_connection(pin_connected_set,
	...
	public boolean connect_to_trace(IntPoint p_from_point, Trace p_to_trace, int p_pen_half_width, int p_cl_type) {
		...
		if (!this.check_polyline_trace(connection_line, trace_layer, p_pen_half_width, p_to_trace.net_no_arr,
				p_cl_type)) {
			return false;
		}
	...
	public boolean remove_trace_tails(int p_net_no, Item.StopConnectionOption p_stop_connection_option) {
		...
		for (Item curr_item : stub_set) {
			...
			if (item_contact_count == 1) {
				...
				// the connected items are no stubs for example if a via is only
				// connected on 1 layer,
				...
				if (curr_ob instanceof Via) {
					...
					for (int i = 0; i < 2; ++i) {
						for (int curr_net_no : curr_item.net_no_arr) {
							...
							if (!pin_found) // at tie pins traces may have
											// different nets
							{
								for (Item curr_contact : contacts) {
