...
public class BasicBoard implements java.io.Serializable {
	...
	public PolylineTrace insert_trace_without_cleaning(Polyline p_polyline, int p_layer, int p_half_width,
		...
		PolylineTrace new_trace = new PolylineTrace(p_polyline, p_layer, p_half_width, p_net_no_arr, p_clearance_class,
				0, 0, p_fixed_state, this);
	...
	public ComponentObstacleArea insert_component_obstacle(Area p_area, int p_layer, int p_clearance_class,
		...
		ComponentObstacleArea obs = new ComponentObstacleArea(p_area, p_layer, Vector.ZERO, 0, false, p_clearance_class,
				0, 0, null, p_fixed_state, this);
		insert_item(obs);
		return obs;
	}
	...
	public void remove_item(Item p_item) {
		...
		additional_update_after_change(p_item); // must be called before p_item
												// is deleted.
	...
	public boolean combine_traces(int p_net_no) {
		...
		while (something_changed) {
			...
			for (;;) {
				...
				if ((p_net_no < 0 || curr_item.contains_net(p_net_no)) && curr_item instanceof Trace
	...
	public boolean normalize_traces(int p_net_no) {
		...
		while (something_changed) {
			...
			for (;;) {
				...
				if (curr_item.contains_net(p_net_no) && curr_item instanceof PolylineTrace
						&& curr_item.is_on_the_board()) {
	...
	public boolean check_trace_shape(TileShape p_shape, int p_layer, int[] p_net_no_arr, int p_cl_class,
		...
		if (default_tree.is_clearance_compensation_used()) {
			...
			default_tree.overlapping_tree_entries_with_clearance(p_shape, p_layer, ignore_net_nos, p_cl_class,
					tree_entries);
		}
		for (TreeEntry curr_tree_entry : tree_entries) {
			...
			if (is_obstacle && (curr_item instanceof PolylineTrace) && p_contact_pins != null) {
				// check for traces of foreign nets at tie pins, which will be
				// ignored inside the pin shape
				...
				for (Pin curr_contact_pin : p_contact_pins) {
					...
					if (intersection == null) {
						TileShape obstacle_trace_shape = curr_item
								.get_tile_shape(curr_tree_entry.shape_index_in_object);
	...
	public boolean check_polyline_trace(Polyline p_polyline, int p_layer, int p_pen_half_width, int[] p_net_no_arr,
			int p_clearance_class) {
		Trace tmp_trace = new PolylineTrace(p_polyline, p_layer, p_pen_half_width, p_net_no_arr, p_clearance_class, 0,
				0, FixedState.UNFIXED, this);
		...
		for (int curr_priority = Drawable.MIN_DRAW_PRIORITY; curr_priority <= Drawable.MIDDLE_DRAW_PRIORITY; ++curr_priority) {
			...
			for (;;) {
				try {
					...
					// may happen when window are changed interactively while
					// running a logfile
					return;
				}
			}
