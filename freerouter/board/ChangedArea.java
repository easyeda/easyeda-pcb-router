...
class ChangedArea {
	...
	private static class MutableOctagon {
		...
		IntOctagon to_int() {
			...
			return new IntOctagon((int) Math.floor(lx), (int) Math.floor(ly), (int) Math.ceil(rx), (int) Math.ceil(uy),
					(int) Math.floor(ulx), (int) Math.ceil(lrx), (int) Math.floor(llx), (int) Math.ceil(urx));
		}
	}
}