...
public class Pin extends DrillItem implements java.io.Serializable {
	...
	public Point get_center() {
		...
		if (pin_center == null) {
			...
			// check that the pin center is inside the pin shape and correct it
			// eventually
	...
	public Item copy(int p_id_no) {
		...
		return new Pin(get_component_no(), this.pin_no, curr_net_no_arr, clearance_class_no(), p_id_no,
	...
	public Shape get_shape(int p_index) {
		...
		if (this.precalculated_shapes == null) {
			// all shapes have to be calculated at once, because otherwise
			// calculation
			...
			for (int index = 0; index < this.precalculated_shapes.length; ++index) {
				...
				if (component_rotation % 90 == 0) {
					int component_ninety_degree_factor = ((int) component_rotation) / 90;
					if (component_ninety_degree_factor != 0) {
						translated_shape = (ConvexShape) translated_shape.turn_90_degree(component_ninety_degree_factor,
								Point.ZERO);
					}
				} else {
					translated_shape = (ConvexShape) translated_shape.rotate_approx(Math.toRadians(component_rotation),
							FloatPoint.ZERO);
				}
				if (!component.placed_on_front() && board.components.get_flip_style_rotate_first()) {
	...
	public java.util.Collection<TraceExitRestriction> get_trace_exit_restrictions(int p_layer) {
		...
		// setting 1.5 to a higher factor may hinder the shove algorithm of the
		// autorouter between
		// the pins of SMD components, because the channels can get blocked by
		// the shove_fixed stubs.

		Component component = board.components.get(this.get_component_no());
		if (component != null) {
			if (component.get_package().pin_count() <= 3) {
				pad_xy_factor *= 2; // allow connection to the longer side also
									// for shorter pads.
			}
		}

		java.util.Collection<Direction> padstack_exit_directions = this.get_padstack()
		...
		for (Direction curr_padstack_exit_direction : padstack_exit_directions) {
			...
			if (curr_rotation_in_degree % 45 == 0) {
				...
				double curr_angle_in_radian = Math.toRadians(curr_rotation_in_degree)
						+ curr_padstack_exit_direction.angle_approx();
				curr_exit_direction = Direction.get_instance_approx(curr_angle_in_radian);
			}
			// calculate the minimum line length from the pin center into
			// curr_exit_direction
			...
			FloatPoint nearest_border_point = curr_exit_line
					.intersection_approx(pad_shape.border_line(intersecting_border_line_no));
			TraceExitRestriction curr_exit_restriction = new TraceExitRestriction(curr_exit_direction,
					center_approx.distance(nearest_border_point));
	...
	public java.util.Set<Pin> get_swappable_pins() {
		...
		// look up all part pins with the same gate_name and the same
		// gate_pin_swap_code
	...
	Direction calc_nearest_exit_restriction_direction(Polyline p_trace_polyline, int p_trace_half_width, int p_layer) {
		java.util.Collection<Pin.TraceExitRestriction> trace_exit_restrictions = this
				.get_trace_exit_restrictions(p_layer);
		...
		FloatPoint trace_entry_location_approx = p_trace_polyline.arr[latest_entry_tuple[0]]
				.intersection_approx(offset_pin_shape.border_line(latest_entry_tuple[1]));
		// calculate the nearest legal pin exit point to
		// trace_entry_location_approx
		...
		for (Pin.TraceExitRestriction curr_exit_restriction : trace_exit_restrictions) {
			...
			if (curr_exit_corner_distance + TOLERANCE < min_exit_corner_distance) {
				...
				// the distances are near equal, compare to the previous corners
				// of p_trace_polyline
