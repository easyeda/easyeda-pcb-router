...
class PullTightAlgo90 extends PullTightAlgo {
	...
	public PullTightAlgo90(RoutingBoard p_board, int[] p_only_net_no_arr, Stoppable p_stoppable_thread,
		...
		for (int i = 5; i <= p_polyline.arr.length; ++i) {
			...
			if (in_clip_shape) {
				...
				if (skip_lines) {
					...
					skip_lines = board.check_trace_shape(shape_to_check, curr_layer, curr_net_no_arr, curr_cl_type,
							this.contact_pins);
				}
				if (skip_lines) {
