...
public class UndoableObjects implements java.io.Serializable {
	...
	public boolean delete(UndoableObjects.Storable p_object) {
		...
		if (curr_delete_list != null) {
			if (object_node.level < this.stack_level) {
				...
				// add curr_ob.undo_object to the current delete list to make
				// Undo possible.
	...
	public boolean redo(Collection<UndoableObjects.Storable> p_cancelled_objects,
		...
		while (it.hasNext()) {
			...
			if (curr_node.redo_object != null && curr_node.redo_object.level == this.stack_level) {
				// Object was created on a lower level and changed on the
				// currenzt level,
				// replace the lower level object by the object on the current
				// layer.
				...
				// Object was created on the current level, allow it to be
				// restored.
		...
		while (it2.hasNext()) {
			...
			if (p_restored_objects == null || !p_restored_objects.remove(curr_deleted_node.object)) {
				// the object needs only be cancelled if it is already in the
				// board
				if (p_cancelled_objects != null) {
	...
	public boolean pop_snapshot() {
		...
		if (deleted_objects_stack_size >= 2) {
			// join the top delete list with the delete list of the second top
			// level
			Collection<UndoableObjectNode> from_delete_list = deleted_objects_stack
					.elementAt(deleted_objects_stack_size - 1);
			Collection<UndoableObjectNode> to_delete_list = deleted_objects_stack
					.elementAt(deleted_objects_stack_size - 2);
	...
	public static class UndoableObjectNode implements java.io.Serializable {
		...
		UndoableObjectNode undo_object; // the object to restore in an undo or
										// null.
		UndoableObjectNode redo_object; // the object to restore in a redo or
										// null.
	}
}
