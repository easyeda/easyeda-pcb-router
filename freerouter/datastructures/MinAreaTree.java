...
//		SearchTreeObject curr_object = (SearchTreeObject) p_leaf.object;
...
//		}
		...
		// Update the parent pointers of the old leaf and new leaf to point to
		// new node
	...
	private final Leaf position_locate(TreeNode p_curr_node, Leaf p_leaf_to_insert) {
		...
		while (!(curr_node instanceof Leaf)) {
			...
			// Choose the the child, so that the area increase of that child
	...
	public void remove_leaf(Leaf p_leaf) {
		...
		while (node_to_recalculate != null) {
			RegularTileShape new_bounds = node_to_recalculate.second_child.bounding_shape
					.union(node_to_recalculate.first_child.bounding_shape);
			if (new_bounds.contains(node_to_recalculate.bounding_shape)) {
				// the new bounds are not smaller, no further recalculate
				// nessesary
				break;
