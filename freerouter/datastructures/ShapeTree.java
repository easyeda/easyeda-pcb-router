...
//		SearchTreeObject curr_object = (SearchTreeObject) p_object;
//		if (curr_object instanceof DrillItem && ((DrillItem) curr_object).net_no_arr.length == 0
//				&& !((DrillItem) curr_object).get_fixed_state().equals(FixedState.SYSTEM_FIXED)) {
//			return null;
//		}
	...
	public Leaf[] to_array() {
		...
		for (;;) {
			...
			++curr_index;
			// go up until parent.second_child != curr_node, which means we came
			// from first_child
	...
	/**
	 * the fixed directions for calculating bounding RegularTileShapes of shapes to
	 * store in this tree.
	 */
	...
	public interface Storable extends Comparable<Object> {
		...
		/**
		 * Get the Shape of this object with index p_index stored in the ShapeTree with
		 * index identification number p_tree_id_no
		 */
		TileShape get_tree_shape(ShapeTree p_tree, int p_index);

		/**
		 * Stores the entries in the ShapeTrees of this object for better performance
		 * while for example deleting tree entries. Called only by insert methods of
		 * class ShapeTree.
		 */
	...
	//////////////////////////////////////////////////////////
	/**
	 * Desscription of an inner node of the tree, which implements a fork to its two
	 * children.
	 */
