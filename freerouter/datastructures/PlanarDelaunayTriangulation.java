...
public class PlanarDelaunayTriangulation {
	...
	public PlanarDelaunayTriangulation(Collection<PlanarDelaunayTriangulation.Storable> p_object_list) {
		...
		// create a big triangle contaning all corners in the list to start
		// with.
	...
	private boolean split(Triangle p_triangle, Corner p_corner) {
		...
		if (containing_edge == null) {
			...
			// split this triangle and the neighbour triangle into 4 new
			// triangles by adding edges from
			...
			// There are exact four new triangles with the first 2 dividing
			// p_triangle and
	...
	public static class ResultEdge {
		private ResultEdge(Point p_start_point, PlanarDelaunayTriangulation.Storable p_start_object, Point p_end_point,
	...
	private class Edge implements Comparable<Edge> {
		 ...
		 */
		public Edge flip() {
			// Create the flipped edge, so that the start corner of this edge is
			// on the left
			// and the end corner of this edge on the right.
			Edge flipped_edge = new Edge(this.right_triangle.opposite_corner(this),
					this.left_triangle.opposite_corner(this));
	...
	private class Triangle {
		...
		public boolean contains(Corner p_corner) {
			...
			for (int i = 0; i < 3; ++i) {
				...
				// checking curr_edge.left_triangle == this instead will not
				// work, if this triangle is an inner node.
				{
					if (curr_side == Side.ON_THE_RIGHT) {
						return false;
					}
		...
		public void get_leaf_edges(Set<Edge> p_result_edges) {
			if (this.is_leaf()) {
				...
				for (Triangle curr_child : this.children) {
					if (curr_child.first_parent == this) // to prevent
															// traversing nodes
															// more than once
					{
		...
		public Triangle[] split_at_border_point(Corner p_corner, Triangle p_neighbour_to_split) {
			...
			// Construct the new edge lines that 2 split triangles of this
			// triangle
			...
			// first split
			// triangle lies on the left side, and the second split triangle on
			// the right side.
			...
			// construct the splitting edge line of p_neighbour_to_split, so
		...
		public boolean validate() {
			...
			if (this.is_leaf()) {
				...
				for (int i = 0; i < 3; ++i) {
					...
					// Check, if the ens corner of the previous line equals to
					// the start corner of this line.
				...
				for (Triangle curr_child : this.children) {
					if (curr_child.first_parent == this) // to avoid traversing
															// nodes more than
															// once.
					{
						curr_child.validate();
